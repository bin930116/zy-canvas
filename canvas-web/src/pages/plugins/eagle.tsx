...
export default function EagleLibraryPage() {
    ...
    const handleCreateFolder = async () => {
        ...
        try {
            ...
            setFolderName("");
            message.success("已在" + (currentFolder?.name || "Eagle 资产库") + "中新建文件夹");
            await load();
        } catch (reason) {
    ...
    if (hydrated && !enabled) {
        return (
            <WorkspacePage grid className="library-page eagle-library-page">
                <PageHeader
                    title="Eagle 资产库"
                    description={`把 Eagle 作为${brandName}的外部资产来源，直接浏览和管理 Eagle 原始文件。`}
                    actions={<Button icon={<ArrowLeft className="size-3.5" />} onClick={() => navigate("/assets")}>返回{brandName}资产库</Button>}
                />
                <section className="mt-4 library-card-surface flex min-h-72 flex-col items-center justify-center rounded-[var(--r-xl)] px-6 py-10 text-center">
                    ...
                    <h2 className="mt-4 text-base font-semibold">先启用 Eagle 资产来源</h2>
                    <p className="mt-2 max-w-md text-sm leading-6 text-foreground/55">启用后，这里会直接显示 Eagle 原本的文件夹和文件，不会把资产复制成{brandName}本地资产。</p>
        ...
        <>
            <WorkspacePage grid className="library-page assets-library-page canvas-library-page eagle-library-page">
                <div className="studio-band">
                    <PageHeader
                        title="Eagle 资产库"
                        description={`Eagle 是${brandName}的外部资产来源；这里复用${brandName}资产库的浏览方式，直接读取和写入 Eagle 原始文件。`}
                        ...
                        actions={(
                            <div className="assets-header-actions">
                                <div className="assets-header-action-buttons">
                                    <Button className="library-primary-action" type="primary" icon={<Upload className="size-3.5" />} onClick={() => fileInputRef.current?.click()} disabled={working}>写入资产</Button>
                                    <Button icon={<ArrowLeft className="size-3.5" />} onClick={() => navigate("/assets")}>{brandName}资产库</Button>
                    ...
                    >
                        <Input allowClear className="w-full sm:w-80" prefix={<Search className="size-4 text-foreground/40" />} value={keyword} placeholder="搜索 Eagle 资产标题、标签或文件夹" onChange={(event) => { setPage(1); setKeyword(event.target.value); }} onPressEnter={() => void load()} />
                    </ListToolbar>
                </div>
                    ...
                    <div className="grid min-h-0 gap-4 lg:grid-cols-[176px_minmax(0,1fr)]">
                        <aside className="eagle-folder-sidebar thin-scrollbar flex gap-2 overflow-x-auto py-3 lg:sticky lg:top-0 lg:block lg:max-h-[calc(100vh-190px)] lg:overflow-y-auto lg:pr-3">
                            ...
                            <button type="button" className={"assets-filter-item " + (selectedFolder === "" ? "is-active" : "")} aria-pressed={selectedFolder === ""} onClick={() => handleFolderSelect("root")}>
                                <span className="assets-filter-item-label">全部资产</span>
                        ...
                        <section className="min-w-0">
                            <nav aria-label="Eagle 文件夹路径" className="mb-3 flex min-w-0 items-center gap-1 text-xs text-foreground/48">
                                <button type="button" className="truncate rounded px-1.5 py-1 hover:bg-surface-hover" onClick={() => handleFolderSelect("root")}>Eagle 资产库</button>
                            ...
                            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <div className="flex flex-wrap items-center gap-2">
                                        <h2 className="text-base font-semibold">{currentFolder?.name || "全部资产"}</h2>
                                        <span className="app-projects-header-meta">{items.length} 个资产</span>
                                    </div>
                                    <p className="mt-1 text-xs text-foreground/48">{currentFolder ? `当前文件夹由 Eagle 管理，${brandName}只负责展示和调用。` : "当前展示 Eagle 资产库中的全部文件。"}</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-2">
...
function EagleItemCard({ item, selected, onOpen }: { item: ExternalAssetItem; selected: boolean; onOpen: () => void }) {
    return <AssetLibraryCard selected={selected}>
        <AssetLibraryCardMedia className={item.kind === "image" || item.kind === "video" ? "assets-cover" : "assets-cover is-light"}>
            <button type="button" className="assets-cover-link" onClick={onOpen} aria-label={"查看 Eagle 资产：" + item.title}>
...
function EagleAssetDrawer({ item, onClose, totalBytes }: { item: ExternalAssetItem | null; onClose: () => void; totalBytes: number }) {
    return <Drawer className="library-drawer" title="资产档案" open={Boolean(item)} size="large" onClose={onClose}>
        {item ? <div className="space-y-4">
            ...
            <div className="grid gap-2">
                ...
                <EagleFact label="资产库总大小" value={formatBytes(totalBytes)} />
            </div>
