...
import { ArrowRight, BookOpenText, FileText, FolderKanban, Images, LayoutGrid, Palette, Plus, Search, Sparkles, Trash2, UsersRound } from "lucide-react";
...
import { createProject, deleteProject, importProjectUnits, listProjects, updateProjectTeam, type ProjectSummary } from "@/services/api/projects";
...
export default function ProjectsPage() {
    ...
    const [teamLinkTarget, setTeamLinkTarget] = useState<ProjectSummary | null>(null);
    const [teamLinkValue, setTeamLinkValue] = useState("");
    ...
    const confirmDeleteProject = (projectId: string, name: string) => {
        modal.confirm({
            title: "删除项目",
            content: `确定删除「${name}」吗？项目章节、画布关联和资产归属将一并移除；独立画布与资产库原始资产会保留。此操作不可撤销。`,
            okText: "删除",
    ...
    const teamsQuery = useQuery({ queryKey: ["teams", "project-link"], queryFn: () => listTeams() });
    const teams = teamsQuery.data?.records || [];
    const teamNameById = useMemo(() => new Map(teams.map((team) => [team.id, team.name])), [teams]);
    const openTeamLink = (row: ProjectSummary) => {
        setTeamLinkTarget(row);
        setTeamLinkValue(row.project.teamId || "");
    };
    const teamLinkMutation = useMutation({
        mutationFn: ({ projectId, teamId }: { projectId: string; teamId: string }) => updateProjectTeam(projectId, teamId),
        onSuccess: (_result, variables) => {
            setTeamLinkTarget(null);
            void queryClient.invalidateQueries({ queryKey: ["projects"] });
            message.success(variables.teamId ? "项目已关联团队" : "已取消关联团队");
        },
        onError: (error) => message.error(error instanceof Error ? error.message : "关联团队失败"),
    });
    ...
    const rows = useMemo(() => {
        ...
        return [...allProjects]
            ...
            .filter(({ project }) => teamFilter === "all" || (teamFilter === "personal" ? !project.teamId : project.teamId === teamFilter))
    ...
    }, [allProjects, keyword, sort, status, teamFilter]);
    ...
    return (
        <WorkspacePage className="library-page" grid>
            ...
            <ListToolbar className="library-toolbar" active={Boolean(keyword || status !== "all" || sort !== "updated" || teamFilter !== "all")} onReset={() => { setKeyword(""); setStatus("all"); setSort("updated"); setTeamFilter("all"); }}>
                ...
                <Select className="w-36" value={teamFilter} onChange={setTeamFilter} placeholder="所属归属" options={[{ label: "全部归属", value: "all" }, { label: "个人项目", value: "personal" }, ...teams.map((team) => ({ label: team.name, value: team.id }))]} />
            ...
            {!query.isLoading && !hasInitialError && rows.length ? (
                <CollectionGrid className="library-grid project-library-grid">
                    {rows.map((row) => <ProjectRow key={row.project.id} row={row} teamName={row.project.teamId ? teamNameById.get(row.project.teamId) : undefined} onDelete={() => confirmDeleteProject(row.project.id, row.project.name)} onLinkTeam={() => openTeamLink(row)} />)}
                </CollectionGrid>
            ) : null}
            ...
            <Modal
                ...
                footer={null}
                destroyOnHidden
                onCancel={() => setTeamLinkTarget(null)}
                width={480}
                styles={{ body: { paddingTop: 12 } }}
            >
                <p className="mb-4 text-xs leading-5 text-foreground/48">
                    将「{teamLinkTarget?.project.name}」关联到团队后，团队成员可以查看并共同编辑这个项目；选择「个人项目」则仅自己可见。
                </p>
                <Radio.Group className="team-link-cards" value={teamLinkValue} onChange={(event) => setTeamLinkValue(event.target.value)}>
                    <Radio value="" className="team-link-card">
                        <span className="team-link-card-icon"><FolderKanban className="size-3.5" /></span>
                        <span className="flex min-w-0 flex-col gap-0.5">
                            <span className="text-[13px] font-medium">个人项目</span>
                            <span className="text-xs text-foreground/45">仅自己可见</span>
                        </span>
                    </Radio>
                    {teams.map((team) => (
                        <Radio key={team.id} value={team.id} className="team-link-card">
                            <span className="team-link-card-icon"><UsersRound className="size-3.5" /></span>
                            <span className="flex min-w-0 flex-col gap-0.5">
                                <span className="truncate text-[13px] font-medium">{team.name}</span>
                                <span className="text-xs text-foreground/45">{team.memberCount ? `${team.memberCount} 位成员` : "团队成员可协作"}</span>
                            </span>
                        </Radio>
                    ))}
                </Radio.Group>
                {teams.length === 0 ? <div className="mb-2 rounded-md border border-border/70 bg-foreground/[.03] px-3 py-2.5 text-xs text-foreground/50">还没有可关联的团队，可以先到「团队」页创建。</div> : null}
                <div className="mt-5 flex justify-end gap-2">
                    <Button onClick={() => setTeamLinkTarget(null)}>取消</Button>
                    <Button type="primary" loading={teamLinkMutation.isPending} disabled={!teamLinkTarget || teamLinkValue === (teamLinkTarget?.project.teamId || "")} onClick={() => teamLinkTarget && teamLinkMutation.mutate({ projectId: teamLinkTarget.project.id, teamId: teamLinkValue })}>
                        保存关联
                    </Button>
                </div>
            </Modal>
...
function ProjectRow({ row, teamName, onDelete, onLinkTeam }: { row: ProjectSummary; teamName?: string; onDelete: () => void; onLinkTeam: () => void }) {
    ...
    return (
        <Link to={`/projects/${row.project.id}/overview`} className="library-card project-library-card group">
            <span className="project-library-cover">
                ...
                <button
                    type="button"
                    className="project-library-cover-action"
                    title={row.project.teamId ? "更换团队" : "关联团队"}
                    aria-label={`关联团队 ${row.project.name}`}
                    onClick={(event) => {
            ...
            <span className="project-library-body">
                <span className="project-library-heading"><strong title={row.project.name}>{row.project.name}</strong>{teamName ? <em className="project-library-team" title={`团队：${teamName}`}><UsersRound className="size-3" />{teamName}</em> : null}{row.project.status === "archived" ? <em>已归档</em> : null}<ArrowRight className="project-library-arrow size-4" /></span>
