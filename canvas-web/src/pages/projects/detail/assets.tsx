...
const pickerCategoryLabels = { all: "全部资产", ...categoryLabels };
...
export default function ProjectAssetsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const audioPickerItems = useMemo<AssetLibraryPickerItem[]>(() => {
        const localItems = personalAssets.flatMap((asset) => {
            ...
            return [{ id: asset.id, title: asset.title, category: "audio", kindLabel: `${characterVoiceFormatName(asset.data.mimeType)} 音频`, asset, description: asset.note || "资产库音频", searchText: (asset.tags || []).join(" ") }];
        });
        const projectItems = assets.flatMap((asset) => {
            ...
            return [{ id: asset.id, title: asset.title, category: "audio", kindLabel: "音频资产", description: asset.previewText || "项目音频资产", searchText: asset.title, disabledReason: undefined, folderId: asset.folderId, imageUrl: undefined }];
        });
    ...
    const addMutation = useMutation({
        mutationFn: async ({ ids, nextFolderId }: { ids: string[]; nextFolderId?: string }) => {
            const result = await linkSelectedProjectAssets(ids, async (id) => {
                ...
                if (!selected) throw new Error("所选资产已不存在，请重新选择");
                return linkProjectAsset(detail.project.id, {
                    assetId: selected.id,
        ...
        onSuccess: ({ assets, failedCount }) => {
            ...
            setAddOpen(false);
            refreshProject();
            if (failedCount) message.warning(`已引用 ${assets.length} 个资产，${failedCount} 个资产引用失败`);
            else message.success(`已引用 ${assets.length} 个资产`);
        },
    ...
    const generateMutation = useMutation({
        ...
        mutationFn: async (asset: ProjectAsset) => {
            ...
            const model = resolveProjectGenerationModel(detail.project, effectiveConfig.imageModel || effectiveConfig.model, "image");
            // 图片尺寸跟随项目默认画幅（后端会归一化为 宽x高）；未设项目画幅时回退全局图片尺寸
            const config = { ...effectiveConfig, model, size: detail.project.aspectRatio || effectiveConfig.size };
            // 图片模型可由后端数据库配置直连，前端渠道非必需（后端会校验 baseUrl/apiKey 并报错）
            if (!model.trim()) throw new Error("请先在设置中配置可用的图片模型");
    ...
    const openCharacterEditor = async (asset: ProjectAsset | "new") => {
        setEditorAsset(asset);
        if (asset === "new") {
            form.setFieldsValue({ name: "", ...Object.fromEntries(characterFields.map(([key]) => [key, ""])) } as CharacterForm);
        } else {
            // 获取完整的角色卡信息
            try {
                const result = await getProjectCharacter(detail.project.id, asset.id);
                const definition = result.character?.definition || {};
                form.setFieldsValue({
                    name: result.asset.title,
                    ...Object.fromEntries(characterFields.map(([key]) => [key, fieldValue(definition[key])]))
                } as CharacterForm);
            } catch (error) {
    ...
    const downloadPreviewAsset = (asset: ProjectAsset) => {
        ...
        if (cover && cover.url && (cover.url.startsWith("http://") || cover.url.startsWith("https://"))) saveAs(cover.url, `${asset.title || "character"}.png`);
        else if (asset.storageKey && (asset.storageKey.startsWith("http://") || asset.storageKey.startsWith("https://"))) saveAs(asset.storageKey, `${asset.title || "character"}.png`);
        else if (cover) saveAs(resourceFileUrl(cover.resourceId), `${asset.title || "character"}.png`);
        else {
        ...
        <div>
            <header className="flex min-h-[72px] flex-col gap-4 pb-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex shrink-0 items-center gap-1.5 sm:pl-4">
                    ...
                    <Button type="text" className="!h-9 !px-3" icon={<Link2 className="size-3.5" />} onClick={() => setAddOpen(true)}>引用资产</Button>
            ...
            <div className="project-assets-layout mt-3 grid gap-3">
                <nav className="space-y-0.5 pr-2" aria-label="资产目录与资产分类">
                    <div className="mb-1 flex h-8 items-center justify-between px-2 text-[var(--fs-tiny)] font-medium text-foreground/42"><span>资产目录</span><button type="button" className="rounded p-1 hover:bg-surface-hover" aria-label="新建根目录文件夹" onClick={() => openFolderEditor(undefined, "")}><FolderPlus className="size-3.5" /></button></div>
                    <button type="button" onClick={() => selectFolder("")} className={`flex h-11 w-full items-center gap-2 rounded-md px-2 text-left text-xs ${folderId === "" ? "bg-surface-active font-medium" : "text-foreground/55 hover:bg-surface-hover"}`}><FolderOpen className="size-4 shrink-0" /><span className="min-w-0 flex-1 truncate">资产库</span><span className="text-[var(--fs-tiny)] tabular-nums text-foreground/42">{folderCountMap[""] || 0}</span></button>
                ...
                <div className="min-w-0">
                    <div className="mb-3 flex min-h-9 flex-wrap items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-1 text-xs text-foreground/48">
                            <button type="button" className="truncate rounded px-1.5 py-1 hover:bg-surface-hover" onClick={() => selectFolder("")}>资产库</button>
                    ...
                    {childFolders.length ? <div className="project-asset-folder-grid mb-5">{childFolders.map((folder) => <ProjectAssetFolderCard key={folder.id} folder={folder} folders={assetFolders} assets={assets} folderCounts={folderCountMap} personalAssets={personalAssets} onOpen={() => selectFolder(folder.id)} onRename={() => openFolderEditor(folder)} onMove={(parentId) => moveFolderMutation.mutate({ id: folder.id, parentId })} onStyle={(style) => styleFolderMutation.mutate({ id: folder.id, style })} onTheme={(theme) => themeFolderMutation.mutate({ id: folder.id, theme })} onDelete={() => modal.confirm({ title: `删除文件夹“${folder.name}”？`, content: "仅空文件夹可以删除，资产和子文件夹不会被级联删除。", okText: "删除", okButtonProps: { danger: true }, cancelText: "取消", onOk: () => deleteFolderMutation.mutateAsync(folder.id) })} deleting={(deleteFolderMutation.isPending && deleteFolderMutation.variables === folder.id) || (moveFolderMutation.isPending && moveFolderMutation.variables?.id === folder.id) || (styleFolderMutation.isPending && styleFolderMutation.variables?.id === folder.id) || (themeFolderMutation.isPending && themeFolderMutation.variables?.id === folder.id)} />)}</div> : null}
            ...
            <AssetLibraryPickerModal
                ...
                emptyTitle="没有可引用的资产"
                emptyDescription="本地资产已全部加入项目，或切换到插件来源选择外部资产。"
            ...
            <AssetLibraryPickerModal
                ...
                categoryLabels={{ all: "全部音频", audio: "声音资产" }}
                initialCategory="audio"
                multiple={false}
                eyebrow="声音资产"
                title="从资产库选择"
                confirmLabel={() => "使用这份声音"}
                emptyTitle="资产库还没有可用音频"
                emptyDescription="可以从底部上传声音资产，上传后会自动选中。"
                upload={{ accept: CHARACTER_VOICE_UPLOAD_ACCEPT, description: `支持 ${CHARACTER_VOICE_FORMAT_LABEL}；上传后保存到资产库`, onUpload: async (files) => {
                    const ids: string[] = [];
                    for (const file of Array.from(files)) {
                        if (!isSupportedCharacterVoiceFile(file)) throw new Error(`声音资产支持 ${CHARACTER_VOICE_FORMAT_LABEL}`);
                ...
                onConfirm={(ids) => {
                    ...
                    if (!resourceId) throw new Error("所选声音资产尚未同步到服务端资源库");
            ...
            />
            <Modal className="workspace-modal workspace-modal-compact" title={`绑定声音资产 · ${voiceAsset?.title || ""}`} open={Boolean(voiceAsset)} okText="绑定并生成新版本" cancelText="取消" okButtonProps={{ loading: bindVoiceMutation.isPending, disabled: !voiceSample }} onCancel={() => { setVoiceAsset(null); setVoiceSample(null); }} onOk={() => bindVoiceMutation.mutate()}>
                <div className="grid gap-3">
                    <div className="rounded-md border border-border/70 bg-foreground/[.025] p-3">
                        <div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="text-[var(--fs-label)] text-foreground/48">当前声音资产</div><div className="mt-1 truncate text-sm font-medium">{voiceSample?.name || "尚未选择声音资产"}</div></div><Button icon={<FolderOpen className="size-3.5" />} onClick={() => setVoicePickerOpen(true)}>选择或上传音频</Button></div>
                        {voiceSample ? <audio className="mt-3 w-full" src={voiceSample.url} controls preload="metadata" /> : <div className="mt-2 text-[var(--fs-tiny)] text-foreground/42">从资产库选择已有音频，或上传 {CHARACTER_VOICE_FORMAT_LABEL} 格式的声音样本。</div>}
                    </div>
                    ...
                    {voiceAsset?.character && voiceAsset.character.voiceStatus !== "missing" ? <div className="flex items-center justify-between pt-1"><span className="text-[var(--fs-label)] text-foreground/45">当前绑定：{voiceAsset.character.voice?.profile.name || "声音资产不可用"}</span><Popconfirm title="解除当前声音绑定？" description="该操作会保留历史版本，并创建一个未绑定声音的新版本。" okText="解除" cancelText="取消" onConfirm={() => unbindVoiceMutation.mutate()}><Button type="text" danger size="small" loading={unbindVoiceMutation.isPending} icon={<VolumeX className="size-3.5" />}>解除声音</Button></Popconfirm></div> : null}
                </div>
            </Modal>
        </div>
    );
...
function projectAssetCanvasPreviewNode(asset: ProjectAsset, personalAsset: Asset | undefined, index: number): CanvasNodeData {
    ...
    const assetHasRemoteUrl = Boolean(asset.storageKey && (asset.storageKey.startsWith("http://") || asset.storageKey.startsWith("https://")));
    const coverUrl = characterCover && characterCover.url && (characterCover.url.startsWith("http://") || characterCover.url.startsWith("https://")) ? characterCover.url : "";
...
function projectAssetRemoteUrl(asset: ProjectAsset) {
    // 生成媒体已直传文件引擎（MINIO），storageKey 即外网 URL，前端可直接加载
    if (asset.storageKey && (asset.storageKey.startsWith("http://") || asset.storageKey.startsWith("https://"))) return asset.storageKey;
    const resourceId = resourceIdFromStorageKey(asset.storageKey);
...
function projectAssetFolderMoveItems(folders: ProjectAssetFolder[], movingFolder: ProjectAssetFolder) {
    ...
    return [
        { key: "move:root", label: "资产库 / 根目录", disabled: !movingFolder.parentId },
...
function ProjectAssetMedia({ asset, personalAsset }: { asset: ProjectAsset; personalAsset?: Asset }) {
    ...
    const remoteUrl = projectAssetRemoteUrl(asset);
    if (asset.mediaType === "image" && remoteUrl) return <CachedResourceImage storageKey={remoteUrl} src={remoteUrl} alt={asset.title} className="h-full w-full bg-black object-cover" fallback={<div className="grid h-full w-full place-items-center bg-black"><MediaIcon kind={asset.mediaType} /></div>} />;
...
function ProjectAssetPreviewModal({ asset, personalAsset, onClose, onDownload, onReplaceImage }: { asset: ProjectAsset | null; personalAsset?: Asset; onClose: () => void; onDownload: () => void; onReplaceImage: () => void }) {
    const characterCover = asset?.character?.representations.find((item) => item.role === "turnaround_sheet") || asset?.character?.representations.find((item) => item.role === "primary") || asset?.character?.representations[0];
    const characterCoverUrl = characterCover?.url && (characterCover.url.startsWith("http://") || characterCover.url.startsWith("https://")) ? characterCover.url : "";
        ...
        <Modal className="workspace-modal workspace-modal-wide library-modal project-asset-preview-modal" title={asset?.category === "character" ? "角色卡预览" : "资产预览"} open={Boolean(asset)} onCancel={onClose} footer={<div className="flex justify-end gap-2"><Button onClick={onClose}>关闭</Button>{asset?.category === "character" ? <Button onClick={onReplaceImage}>替换图片</Button> : null}{canDownload ? <Button type="primary" icon={<Download className="size-3.5" />} onClick={onDownload}>下载</Button> : null}</div>}>
            {asset ? <div className="project-asset-preview-layout">
                <div className={`project-asset-preview-stage ${previewClass}`}>
                    {asset.category === "character" ? characterCover ? <CachedResourceImage key={characterCoverUrl || characterCover.resourceId} storageKey={characterCoverUrl || `resource:${characterCover.resourceId}`} src={characterCoverUrl || resourceFileUrl(characterCover.resourceId)} alt={asset.title} className="project-asset-preview-media" eager fallback={<div className="grid min-h-48 place-items-center text-foreground/35"><UserRound className="size-12" /></div>} /> : <div className="grid min-h-48 place-items-center text-foreground/35"><UserRound className="size-12" /></div> : asset.mediaType === "video" && remoteUrl ? <video src={remoteUrl} controls className="project-asset-preview-media" /> : asset.mediaType === "audio" && remoteUrl ? <audio src={remoteUrl} controls className="project-asset-preview-audio" /> : asset.mediaType === "image" && remoteUrl ? <img key={remoteUrl} src={remoteUrl} alt={asset.title} className="project-asset-preview-media" /> : personalAsset?.kind === "video" ? <video src={personalAsset.data.url} controls className="project-asset-preview-media" /> : personalAsset?.kind === "audio" ? <audio src={personalAsset.data.url} controls className="project-asset-preview-audio" /> : personalAsset?.kind === "image" ? <AssetMediaPreview asset={personalAsset} alt={asset.title} className="project-asset-preview-media" /> : personalAsset?.kind === "text" ? <p className="project-asset-preview-text">{personalAsset.data.content}</p> : asset.mediaType === "text" && asset.previewText ? <p className="project-asset-preview-text">{asset.previewText}</p> : <div className="grid min-h-48 place-items-center text-foreground/35"><MediaIcon kind={asset.mediaType} /></div>}
                </div>
