...
export default function ProjectSettingsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const coverPickerItems = useMemo<AssetLibraryPickerItem[]>(() => {
        ...
        for (const asset of personalAssets) {
            ...
            result.push({ id: asset.id, title: asset.title, category: "image", kindLabel: "图片", asset, description: asset.note || "个人资产库", searchText: (asset.tags || []).join(" ") });
        }
        for (const asset of projectCoverAssets) {
            ...
            if (!resourceId || seenResourceIds.has(resourceId)) continue;
            seenResourceIds.add(resourceId);
            result.push({ id: `project:${asset.id}`, title: asset.title, category: "image", kindLabel: "项目图片", imageUrl: resourceFileUrl(resourceId), description: "项目资产库", searchText: asset.title });
        }
        ...
        <div>
            ...
            <section className="border-t border-border/70 py-5">
                ...
                <div className="flex flex-col gap-3 rounded-lg border border-border/70 bg-surface-active p-3 sm:flex-row sm:items-center">
                    ...
                    <div className="min-w-0 flex-1"><div className="text-sm font-medium">{project.coverResourceId ? "已设置项目主图" : "尚未设置项目主图"}</div><p className="mt-1 text-xs leading-5 text-foreground/48">从个人资产库或项目资产库选择，也可以在选择窗口中上传一张新图片。</p></div>
