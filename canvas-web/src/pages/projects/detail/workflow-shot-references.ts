...
export function buildShotAssetReferenceContext(detail: ProjectDetail, shotId: string): ShotAssetReferenceContext {
    const assetByVersionId = new Map(detail.assets.flatMap((asset) => [[asset.primaryVersionId, asset] as const, [asset.id, asset] as const]));
...
export function resolveShotAssetMentionPrompt(prompt: string, context: ShotAssetReferenceContext, options: { dialogue?: string } = {}) {
    const imageLabelByAssetId = new Map(context.mentionReferences.map((reference, index) => [reference.assetId, `Image ${index + 1}`]));
...
export function compileShotAssetReferencePrompt(references: ShotPromptAssetReference[]) {
    ...
    const lines = references.flatMap((reference) => {
        ...
        const media = [`${visualLabel}：Image ${reference.imageIndex}`, reference.audioIndex ? `声音参考：Audio ${reference.audioIndex}` : ""].filter(Boolean).join("；");
        return [
...
function projectAssetReferenceImage(asset: ProjectAsset, reference?: ShotAssetReference): ReferenceImage | undefined {
    ...
    if (representation) {
        return {
            ...
            type: "image/*",
            dataUrl: "",
            url: representation.url && (representation.url.startsWith("http://") || representation.url.startsWith("https://")) ? representation.url : resourceFileUrl(representation.resourceId),
            storageKey: representation.url && (representation.url.startsWith("http://") || representation.url.startsWith("https://")) ? representation.url : resourceStorageKey(representation.resourceId),
        };
    }
    if (asset.mediaType !== "image" || !asset.storageKey) return undefined;
    const resourceId = resourceIdFromStorageKey(asset.storageKey);
    // storageKey 可能是 resource 键（取 resourceFileUrl）或 http 直链（直接用，供提交时透传）
