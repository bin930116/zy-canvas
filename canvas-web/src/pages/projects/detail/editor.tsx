...
// 剪辑预设插件（editor-shell 等）通过模块副作用注册 editorSlots；不 import 则面板为空
    ...
    return (
        <header className="flex h-12 shrink-0 items-center gap-2 border-b border-[var(--director-sequencer-border)] bg-[var(--director-sequencer-surface-raised)] px-3">
            ...
            <div className="flex items-center gap-1.5">
                {/* 保存状态指示（非按钮）：绿勾=已保存到本地缓存 */}
                <span
                    className="grid size-8 place-items-center"
                    title={saveError ?? (saving ? "保存中" : isDirty ? "有未保存的更改" : "已保存到本机")}
                >
                    {saveError ? (
...
export default function ProjectEditorView({ detail }: { detail: ProjectDetail }) {
    ...
    // 上个会话导入但详情未携带（或导入后未刷新）的资产会在进入编辑器后补现。
    useEffect(() => {
        let cancelled = false;
        void (async () => {
    ...
    // 分镜/导出成片，且 ready + 有资源)尚未作为资产进本项目时，自动
    // linkProjectAsset(source=canvas) 并入资产库 —— 剪辑器“资产选项”直接可用画布产物。
    ...
    useEffect(() => {
        ...
        // 已同步过的(本会话 ref)或已存在于项目资产的(storageKey 命中)均跳过 ——
                ...
                <div className="flex min-h-0 flex-1 flex-col gap-2 p-2">
                    ...
                    <div className="flex min-h-0 flex-1 gap-2">
                        ...
                        <aside className="flex h-full w-64 shrink-0 flex-col overflow-hidden rounded-xl bg-[var(--director-sequencer-surface)] shadow-[0_1px_2px_rgba(0,0,0,0.3)]">
                            ...
                            />
                            {leftTab === "asset" ? (
                                <SlotStack slots={assetSlots} emptyHint="资产库插件未加载。" />
                            ) : (
