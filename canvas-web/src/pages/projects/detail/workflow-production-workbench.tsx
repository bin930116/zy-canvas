...
export default function WorkflowProductionWorkbench(props: Props) {
    ...
    const artifacts = useMemo(() => selectedShot ? (detail.shotArtifacts || [])
        .filter((item) => item.shotId === selectedShot.id && item.type === artifactType && item.status === "ready")
        .slice()
        .sort((left, right) => {
            if (Boolean(right.selected) !== Boolean(left.selected)) return right.selected ? 1 : -1;
    ...
    // 项目设置默认模型优先（可能来自后端 zy_model，不强制出现在前端渠道列表）
    const defaultModel = resolveProjectGenerationModel(
        detail.project,
        generationCapability === "video" ? effectiveConfig.videoModel : effectiveConfig.imageModel,
        generationCapability,
    );
    const initialModel = defaultModel || modelOptions[0] || "";
    const [selectedModel, setSelectedModel] = useState(initialModel);
    const selectedModelRef = useRef(initialModel);
    // 切换 stage（分镜图/视频）时自动切换到对应能力的默认模型
    useEffect(() => {
        setSelectedModel(initialModel);
        selectedModelRef.current = initialModel;
    }, [activeStage, initialModel]);
    // 项目默认 / 全局默认兜底：选择器未同步或被清空时仍按默认提交
    const effectiveSelectedModel = selectedModel?.trim() || initialModel;
    ...
    const routedModel = resolveCompatibleModel(effectiveConfig, effectiveSelectedModel, modelRequirements) || effectiveSelectedModel;
    ...
    useEffect(() => {
        selectedModelRef.current = initialModel;
        setSelectedModel((current) => (current?.trim() ? current : initialModel));
    ...
    const changeAssetBinding = useMutation({
        mutationFn: async ({ asset, reference }: { asset?: ProjectAsset; reference?: ShotAssetReference }) => {
            ...
            const versionId = asset?.primaryVersionId || asset?.id;
            if (!versionId) throw new Error("该资产还没有可绑定版本");
            return linkShotAsset(projectId, selectedShot.id, { assetVersionId: versionId, role: "reference" });
        },
    ...
    const buildStoryboardImagePrompt = (prompt: string, context: typeof shotAssetReferenceContext) => {
        const titles = context.mentionReferences.map((reference) => reference.title).filter(Boolean);
        const line = titles.length ? `画面中出现的角色与资产形象以参考图为准保持一致：${titles.join("、")}。` : "";
        return [prompt.trim(), line].filter(Boolean).join("\n");
    };
    ...
    const generateArtifact = async () => {
        ...
        try {
            ...
            // 生产任务与角色图一致：直接走后端任务队列，任务完成后由后端按 metadata 回填镜头产物。
            // 图片模型配置由后端按 DB 模型表读取（与角色三视图同一链路），前端不再做渠道完整性拦截
            ...
            const basePrompt = mode === "video"
                ...
                : [values.imagePrompt || values.plotDescription, values.action].filter(Boolean).join("\n");
            // 图像生成：不拼"人物参考：图片N / 声音画像"等结构化块（图像模型会把这类文字渲染进画面），
            // 只给一句简短一致性提示；参考图本身已通过 referenceImages 随请求提交。
            const resolvedPrompt = mode === "video"
                ? resolveShotAssetMentionPrompt(basePrompt, shotAssetReferenceContext, { dialogue: values.dialogue })
                : buildStoryboardImagePrompt(basePrompt, shotAssetReferenceContext);
            ...
            await submitBackendGenerationTask({
                ...
                config,
                referenceImages: [
                    // 视频模式：自动把当前镜头的分镜图作为参考图
                    ...(mode === "video" ? (() => {
                        const storyboardArtifact = currentArtifact(detail, submittingShot.id, "storyboard");
                        if (storyboardArtifact?.resourceId) {
                            return [{ url: artifactPreviewUrl(storyboardArtifact) }];
                        }
                        return [];
                    })() : []),
                    ...shotAssetReferenceContext.referenceImages,
                ],
            ...
            // 任务完成后自动轮询刷新，不用手动刷新页面
            const pollTimer = window.setInterval(async () => {
                await onRefresh();
                const task = detail.tasks?.find((t: any) => t.clientContext?.shotId === submittingShot.id);
    ...
    return (
        <div className="workflow-production-shell">
                        ...
                        <div className="workflow-shot-form-scroll thin-scrollbar">
                            ...
                            <Form.Item
                                ...
                                label={
                                    <span className="flex items-center justify-between">
                                        <span>视频提示词</span>
                                        <Button type="link" size="small" icon={<WandSparkles className="size-3.5" />} onClick={() => setOptimizerOpen(true)}>
                                            专业生成
                                        </Button>
                                    </span>
                                }
                            ...
                            >
                                <div className="workflow-settings-section">
                                    ...
                                    {/* 模型跟项目设置默认，不在分镜内单独改；摘要仍展示当前生效模型 */}
                                    <Form.Item label="生成模型">
                                        <ModelPicker
                                            ...
                                            placeholder={defaultModel
                                                ? `项目默认 · ${modelDisplayName(effectiveConfig, defaultModel) || defaultModel}`
                                                : activeStage === "video" ? "选择视频模型" : "选择图片模型"}
                                            showSelectedPrice
                                        />
                                    </Form.Item>
            ...
            >
                {imagePreviewArtifact?.resourceId ? <img className={`workflow-image-preview-modal ${imagePreviewArtifact.type === "action_board" ? "grayscale" : ""}`} src={artifactPreviewUrl(imagePreviewArtifact)} alt={imagePreviewArtifact.type === "action_board" ? "动作预演大图" : "分镜图大图"} /> : null}
            </Modal>
            ...
            <PromptStudioModal
                ...
                onApply={(optimized) => {
                    form.setFieldsValue({ videoPrompt: optimized });
                    setEditorDirty(true);
                }}
                mode="video"
                initialPrompt={form.getFieldValue("videoPrompt") || ""}
                context={{
                    title: form.getFieldValue("title") || undefined,
                    action: form.getFieldValue("action") || undefined,
                    dialogue: form.getFieldValue("dialogue") || undefined,
                    shotSize: form.getFieldValue("shotSize") || undefined,
                    cameraAngle: form.getFieldValue("cameraAngle") || undefined,
                    cameraMovement: form.getFieldValue("cameraMovement") || undefined,
                    duration: Number(form.getFieldValue("durationSeconds")) || undefined,
                    styleHint: [detail.project.name, detail.project.description, detail.units.find((u) => u.id === unitId)?.title].filter(Boolean).join(" · ") || undefined,
                }}
            />
        </div>
    );
...
function BoundAssets({ detail, shotId, changing, onUnlink }: { detail: ProjectDetail; shotId: string; changing: boolean; onUnlink: (reference: ShotAssetReference) => void }) {
    ...
    const assetByVersionId = useMemo(() => new Map(detail.assets.flatMap((asset) => [[asset.primaryVersionId, asset] as const, [asset.id, asset] as const])), [detail.assets]);
    return (
...
function LatestPreview({ artifact, emptyText, onPreviewImage }: { artifact?: ShotArtifact; emptyText: string; onPreviewImage: (artifact: ShotArtifact) => void }) {
    ...
    const src = artifactPreviewUrl(artifact);
...
function VideoArtifactPreview({ src, title }: { src: string; title: string }) {
    ...
    if (playing) return <CachedResourceVideo className="workflow-preview-media" storageKey={src} src={src} controls autoPlay playsInline preload="metadata" aria-label={title} />;
...
function artifactPreviewUrl(artifact: ShotArtifact): string {
    // 1) 产物直链（MinIO/dev 下载链，生成时写入）
    ...
    try {
        ...
        if (meta && typeof meta.url === "string" && /^https?:\/\//i.test(meta.url)) return meta.url;
    } catch {
        // 忽略元数据解析错误
    }
    // 3) 资源服务（仅当有合法 resourceId 且形如 resource 前缀时）
    if (artifact.resourceId && !artifact.resourceId.startsWith("asset")) {
        return resourceFileUrl(artifact.resourceId);
    }
    return "";
}
...
function assetPreviewUrl(asset: ProjectAsset) {
    ...
    if (representation?.url && /^https?:\/\//i.test(representation.url)) return representation.url;
    if (representation?.resourceId) return resourceFileUrl(representation.resourceId);
    if (asset.storageKey && /^https?:\/\//i.test(asset.storageKey)) return asset.storageKey;
