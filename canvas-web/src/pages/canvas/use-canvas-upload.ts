...
export function useCanvasUpload({
    ...
    const persistMediaNode = useCallback(async (node: CanvasNodeData) => {
        try {
            ...
            message.warning(error instanceof Error ? `媒体已添加到画布，但资产同步失败：${error.message}` : "媒体已添加到画布，但资产同步失败");
            return false;
        }
    ...
    const createImageAssetNode = useCallback(async (asset: ImageAsset, position?: Position) => {
        try {
            ...
            message.error(error instanceof Error ? error.message : "资产图片读取失败");
        }
    ...
    const uploadTimelineMedia = useCallback(async (files: File[]): Promise<TimelineDirectMedia[]> => {
        ...
        for (const file of supportedFiles) {
            try {
                ...
                message.error(error instanceof Error ? `资产上传失败：${error.message}` : "资产上传失败");
            }
        }
        if (created.length) message.success(`已上传 ${created.length} 个资产到时间线`);
        return created;
    ...
    const handleAssetsInsert = useCallback(async (payloads: InsertAssetPayload[]): Promise<CanvasNodeData[]> => {
        ...
        return insertAssetPayloads(payloads, origin, `已插入 ${payloads.length} 项资产`, "资产插入失败");
