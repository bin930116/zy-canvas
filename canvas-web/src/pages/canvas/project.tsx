...
function InfiniteCanvasPage() {
    ...
    const archiveNodesToLinkedFolder = useCallback((folder: CanvasNodeData, droppedNodes: CanvasNodeData[]) => {
        ...
        void Promise.all(droppedNodes.map((node) => ensureCanvasNodeAsset({ canvasId: projectId, domainProjectId, folderId, node, source: "canvas-manual" })))
            ...
            .catch((error) => message.error(error instanceof Error ? error.message : "资产归档失败"));
    ...
    useEffect(() => {
        ...
        void loadAssetsForUse(canvasAssetHandoffIds(searchParams)).catch((error) => message.error(error instanceof Error ? error.message : "转入资产读取失败"));
    ...
    const handleLibraryAssetsInsert = useCallback(
        async (payloads: InsertAssetPayload[]) => {
            if (assetInsertScope === "timeline") {
                ...
                if (media.length !== payloads.length) throw new Error("图片和文本资产暂不支持直接入轨，请先插入画布");
    ...
    const handleTimelineProjectAssetsInsert = useCallback(
        async (payloads: InsertAssetPayload[]) => {
            if (projectAssetScope === "timeline") {
                ...
                if (inserted < payloads.length) message.info("图片/文本/角色资产暂不支持直接入轨，仅音视频资产已加入时间线");
                return;
            }
    ...
    // 资产库打开入口：画布作用域（工具栏/空态/侧栏）与时间线作用域（时间线弹窗）分别标记插入目标。
    ...
    const handleProjectFolderInsert = useCallback((folderId: string) => {
        ...
        if (!folder || !linkedProjectId) throw new Error("资产文件夹已不存在，请刷新后重试");
    ...
    const uploadNodeImageToArkPrivateAsset = useCallback(async (node: CanvasNodeData) => {
        ...
        message.loading({ key: feedbackKey, content: "正在保存并上传到方舟资产库...", duration: 0 });
        try {
            ...
            if (!resourceID) {
                ...
                if (!resourceID) throw new Error("图片未能保存到系统资产库，请检查对象存储配置后重试");
            ...
            message.success({ key: feedbackKey, content: "已同步到方舟资产库，Seedance 将自动复用该资产", duration: 4 });
        } catch (error) {
            message.error({ key: feedbackKey, content: error instanceof Error ? error.message : "上传到方舟资产库失败", duration: 5 });
        } finally {
    ...
    const confirmUploadNodeImageToArkPrivateAsset = useCallback((node: CanvasNodeData) => {
        Modal.confirm({
            title: "上传到方舟资产库",
            content: "仅可上传你拥有肖像、版权或其他合法使用权的图片。方舟审核通过后，Seedance 会使用受控资产标识生成视频。",
            okText: "确认拥有使用权并上传",
            cancelText: "取消",
    ...
    const { addScriptRow, createAndGenerateScriptVideos, createScriptActionBoards, createScriptImageNodes, createScriptVideoNodes, generateScriptImages, generateScriptRows, generateScriptVideos, removeScriptRow, replaceScriptRows, updateScriptRow } =
        useCanvasStoryboard({
            projectId,
            project: linkedProjectQuery.data?.project,
            addedSkills,
            nodesRef,
            connectionsRef,
            setNodes,
