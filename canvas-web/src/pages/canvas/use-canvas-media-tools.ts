...
export function useCanvasMediaTools({
    ...
    const persistMediaNodes = useCallback(async (mediaNodes: CanvasNodeData[]) => {
        ...
        for (const mediaNode of mediaNodes) {
            try {
                ...
                message.warning(`媒体节点已创建，但资产库写入失败：${error instanceof Error ? error.message : "未知错误"}`);
            }
        }
        if (assetIds.size > 0) {
    ...
    // 从视频片段提取声音：FFmpeg 提取 MP3 → 上传为音频资源 → 创建音频节点 → 写入资产库/项目资产。
    const runExtractVideoAudio = useCallback(async (node: CanvasNodeData, params: CanvasVideoSegmentParams) => {
        ...
        try {
            ...
            try {
                ...
                progress.done(result.linkedToProject ? "声音已提取并加入资产库与项目资产" : "声音已提取并加入资产库");
            } catch (assetError) {
                progress.done(`声音已提取并生成音频节点，资产库写入失败：${assetError instanceof Error ? assetError.message : "未知错误"}`);
            }
        } catch (error) {
        ...
        try {
            ...
            segmentNodes.forEach((segmentNode) => {
                void ensureCanvasNodeAsset({ canvasId: projectId, domainProjectId, node: segmentNode, source: "canvas-manual" })
                    ...
                    .catch((assetError) => message.warning(`片段已截取，但资产库写入失败：${assetError instanceof Error ? assetError.message : "未知错误"}`));
            });
