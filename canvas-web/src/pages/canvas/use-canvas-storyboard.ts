...
import { resolveProjectGenerationModel } from "@/services/api/projects";
...
type UseCanvasStoryboardOptions = {
    projectId: string;
    project?: { defaultImageModel?: string; defaultVideoModel?: string } | null;
    addedSkills: Skill[];
...
export function useCanvasStoryboard({
    ...
    const createAndGenerateScriptVideos = useCallback(async (nodeId: string, rowIds?: string[]) => {
        const videoModel = resolveProjectGenerationModel(project, effectiveConfig.videoModel || effectiveConfig.model, "video");
    ...
    }, [connectionsRef, confirmGenerationSubmission, createScriptVideoNodes, effectiveConfig, enqueueGenerationBatch, isAiConfigReady, message, nodesRef, project, setConnections, setNodes, setSelectedNodeIds]);
    ...
    }, [connectionsRef, confirmGenerationSubmission, createScriptVideoNodes, effectiveConfig, enqueueGenerationBatch, isAiConfigReady, message, nodesRef, project, setConnections, setNodes]);
