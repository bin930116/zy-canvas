...
export default function CreatePage() {
    ...
    const uploadLibraryAssets = async (files: FileList | File[]) => {
        ...
        if (assetIds.length) toast.success(`${assetIds.length} 个资产已上传到资产库并自动选中`);
        if (failed.length) toast.error(`${failed.length} 个资产上传失败，请重试`);
        return assetIds;
    };
    ...
    const confirmDeleteConversation = (conversation: CreationConversation) => {
        ...
        modal.confirm({
            ...
            title: "删除历史对话？",
            content: `确定删除「${label}」吗？这只会删除历史对话记录，不会删除已上传或生成的任何资产。此操作不可撤销。`,
            okText: "删除对话",
    ...
    return <>
        ...
        {libraryOpen ? <Suspense fallback={null}><AssetLibraryPickerModal
            ...
            upload={{ accept: creationUploadAccept(mode), description: mode === "text" ? "支持图片、视频、音频和常用文档；媒体会保存到资产库" : `支持图片${mode === "video" ? "、视频和音频" : ""}，上传后保存到资产库`, onUpload: uploadLibraryAssets, external: { accept: "image/*", description: "写入当前 Eagle 文件夹；Eagle 当前支持图片文件", onUpload: (files, folderId) => externalAssetSources.uploadExternalFiles(files, folderId) } }}
...
const creationAssetCategoryLabels: Record<string, string> = { all: "全部资产", ...ASSET_CATEGORY_LABELS };
...
function MediaResult({ item, onRetryFailure, onCreateVariant }: { item: CreationMessage; onRetryFailure: () => void; onCreateVariant: () => void }) {
    ...
    const resultAssetIds = storedResultUrls.length || item.taskIds?.length ? creationResultAssetIds(assets, { messageId: item.id, taskIds: item.taskIds || [], resultUrls: storedResultUrls }) : [];
    // 刷新后 blob: 地址会失效：优先用资产库里可播的 http/storage 地址
    const assetResultUrls = resultAssetIds
        .map((id) => assets.find((asset) => asset.id === id))
        .filter((asset): asset is NonNullable<typeof asset> => Boolean(asset))
        .map((asset) => (asset.kind === "image" ? asset.data.dataUrl || asset.coverUrl : asset.kind === "video" || asset.kind === "audio" ? asset.data.url : ""))
        .filter((url): url is string => Boolean(url) && !url.startsWith("blob:"));
    const playableUrls = assetResultUrls.length ? assetResultUrls : storedResultUrls;
    ...
    if ((item.status === "error" || item.status === "cancelled") && !playableUrls.length) return <div className="creation-media-error"><span>{item.status === "cancelled" ? item.content || "已停止" : generationErrorMessage(item.error || "生成失败")}</span><button type="button" onClick={onRetryFailure}><RefreshCw />重新生成</button></div>;
    if (!playableUrls.length) return <div className="creation-media-empty">没有返回可预览结果 <button type="button" onClick={onRetryFailure}>重试</button></div>;
    const isVideo = item.mode === "video";
    return <div className="creation-media-result">
        {isVideo ? <button type="button" className="creation-video-result" onClick={() => { setPreviewType("video"); setPreviewUrl(playableUrls[0]); }} aria-label="预览生成视频"><video muted preload="metadata" src={playableUrls[0]} /><span><Maximize2 />预览视频</span></button> : <div className="creation-image-result-grid">{playableUrls.map((url) => <button key={url} type="button" className="creation-image-result" onClick={() => { setPreviewType("image"); setPreviewUrl(url); }} aria-label="预览生成图片"><img src={url} alt="生成结果" /><span><Maximize2 /></span></button>)}</div>}
        <div className="creation-media-actions"><span>{isVideo ? "视频结果" : `${playableUrls.length} 张图片`}</span><Link to={canvasPath}>{resultAssetIds.length ? "添加到画布" : "打开画布"}</Link>{playableUrls.map((url, index) => <a key={`${url}-download`} href={url} download>{playableUrls.length > 1 ? `下载 ${index + 1}` : <><Download />下载</>}</a>)}</div>
        <CreationMediaPreviewModal url={previewUrl} type={previewType} onClose={() => setPreviewUrl("")} />
    </div>;
}
                ...
                {props.attachments.length || referencesSupported ? <div className={`creation-reference-panel${trackState.isExpanded ? " is-expanded" : ""}`} aria-busy={interactionBusy}>
                    {trackState.isExpanded ? <div className="creation-reference-panel-header">
                        ...
                        <div className="creation-reference-panel-actions">
                            {props.attachments.length ? <button type="button" onClick={props.onClearAttachments} disabled={interactionBusy}>清空全部资产</button> : null}
                            <Tooltip title="收起资产面板"><button type="button" className="creation-reference-panel-collapse" onClick={() => setReferencePanelExpanded(false)} aria-label="收起资产面板"><Minimize2 aria-hidden="true" /></button></Tooltip>
                        </div>
                    </div> : null}
