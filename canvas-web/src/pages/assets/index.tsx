...
import { CachedResourceVideo } from "@/components/cached-resource-video";
...
const assetSourceLabels: Record<string, string> = {
    ...
    uploaded: "手动上传",
    canvas: "画布生成",
    chapter_character_extract: "章节分析",
    agent: "Agent 生成",
    "手动添加": "手动添加",
    "手动上传": "手动上传",
};

function assetSourceLabel(source?: string) {
...
export default function AssetsPage() {
    ...
    const allLibraryAssets = useMemo(() => { const seen = new Set<string>(); return assets.map(normalizeAssetRecord).filter((asset): asset is LibraryAsset => { if (asset.kind === "entity" || seen.has(asset.id)) return false; seen.add(asset.id); return true; }); }, [assets]);
    ...
    const visibleAssets = useMemo(
        () => (assetPageQuery.data?.assets ?? (assetPageQuery.isPending ? [] : localVisibleAssets)).map(normalizeAssetRecord).filter((asset): asset is LibraryAsset => {
            // 过滤掉 data 为空的对象
            if (!asset.data) return false;
            return true;
        }),
        [assetPageQuery.data?.assets, assetPageQuery.isPending, localVisibleAssets],
    );
    ...
    const saveFolder = async () => {
        ...
        try {
            ...
            message.success(folderEditor === "new" ? "资产分类已创建" : "资产分类已重命名");
        } catch (error) {
            message.error(error instanceof Error ? error.message : "资产分类保存失败");
        } finally {
    ...
    const removeFolder = async (folder: AssetFolder) => {
        try {
            ...
            setPage(1);
            await invalidateAssetLibrary();
            message.success(`已删除分类「${folder.name}」，其中资产已移至未分类`);
        } catch (error) {
            message.error(error instanceof Error ? error.message : "资产分类删除失败");
        }
    };
    ...
    const moveAssetsToFolder = async (assetIds: string[], folderId: string) => {
        ...
        try {
            ...
            setSelectedIds([]);
            await invalidateAssetLibrary();
            message.success(`已移动 ${assetIds.length} 个资产`);
        } catch (error) {
            message.error(error instanceof Error ? error.message : "移动资产失败");
        }
    };
    ...
    const saveAsset = async () => {
        ...
        try {
            ...
            message.warning(editingAsset ? "资产已在本地更新，稍后自动同步至云端" : "资产已在本地保存，稍后自动同步至云端");
        }
    ...
    const downloadImage = async (asset: LibraryAsset) => {
        ...
        const rawStorageKey = asset.kind === "image" ? (asset.data?.storageKey || (asset as LibraryAsset & { storageKey?: string }).storageKey || "") : "";
        const url = asset.kind === "image" ? (asset.data?.dataUrl || asset.coverUrl || (rawStorageKey && /^https?:\/\//i.test(rawStorageKey) ? rawStorageKey : "")) : (asset.data?.url || "");
        if (!url) return;
        const mimePart = asset.kind === "model" ? (asset.data?.fileName?.split(".").pop() || "glb") : (asset.data?.mimeType?.split("/")?.[1] || "png");
        const extension = mimePart && mimePart !== "image" ? mimePart : "png";
    ...
    const restoreAsset = async (asset: LibraryAsset) => {
        ...
        try {
            await saveRemoteUserDataNow();
            message.success(`已还原资产「${asset.title}」`);
        } catch {
    ...
    const batchArchive = async () => {
        ...
        try {
            await saveRemoteUserDataNow();
            message.success(`已将 ${count} 个资产移入回收站`);
        } catch {
            message.warning("已移入回收站，稍后自动同步至云端");
        }
    };
    ...
    const emptyTrash = async () => {
        ...
        try {
            ...
            setSelectedIds([]);
            await invalidateAssetLibrary();
            message.success(`已彻底清空回收站 ${count} 个资产`);
        } catch (error) {
    ...
    const confirmBatchDelete = async () => {
        ...
        try {
            ...
            await invalidateAssetLibrary();
            message.success(`已彻底删除 ${selectedAssets.length} 个资产`);
            setSelectedIds([]);
    ...
    return (
        <>
            <WorkspacePage grid className="library-page assets-library-page canvas-library-page">
                <div className="studio-band">
                    <PageHeader
                        ...
                        description={viewMode === "trash" ? "已删除画布或手动归档的临时资产，可随时还原或彻底清理。" : "管理文本、图片、视频、音频和 3D 模型资产。"}
                        meta={<span className="app-projects-header-meta assets-header-meta">{validAssets.length} 个资产</span>}
                        actions={
                                    ...
                                    {viewMode === "trash" ? (
                                        <>
                                            {trashAssets.length > 0 ? (
                                                <Popconfirm
                                                    title="确定清空回收站吗？"
                                                    description="清空后所有回收站资产及其文件将被彻底永久删除，不可恢复。"
                                        ...
                                        <>
                                            ...
                                            <Button title="导出全部资产" aria-label="导出全部资产" icon={<Download className="size-4" />} onClick={() => void exportAllAssets()} />
                                            <Dropdown
                                                trigger={["click"]}
                                                menu={{
                                                    items: [
                                                        { key: "package", icon: <FileUp className="size-4" />, label: "导入资产包", onClick: () => assetInputRef.current?.click() },
                                                        { key: "model", icon: <Upload className="size-4" />, label: "上传 3D 模型", onClick: () => modelInputRef.current?.click() },
                                                    ],
                                                }}
                                            >
                                                <Button title="导入资产" aria-label="导入资产" icon={<FileUp className="size-4" />} />
                                            </Dropdown>
                                        </>
                                    )}
                                </div>
                    ...
                    <div className="grid min-h-0 gap-4 lg:grid-cols-[176px_minmax(0,1fr)]">
                        ...
                        <section className="min-w-0">
                            {viewMode === "trash" ? (
                                <div className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-xs text-amber-600 dark:text-amber-300">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span>{retentionDays > 0 ? `回收站内的资产将在 ${retentionDays} 天后自动彻底清除。您可以随时还原资产，或手动彻底删除释放空间。` : "回收站内的资产当前设置为永久保留，您可以随时还原资产或手动彻底清除。"}</span>
                                    </div>
                                </div>
                            ) : null}
                            ...
                            {validAssets.length === 0 ? (
                                viewMode === "trash" ? (
                                    <WorkspaceState icon="assets" compact title="回收站是空的" description="删除画布或手动移入回收站的资产会暂存到这里，可在需要时随时还原。" />
                                ) : (
                                ...
                                <>
                                    {filteredAssets.length === 0 ? (
                                        <WorkspaceState icon="assets" compact title="没有匹配的资产" description="调整关键词或左侧分类后再试。" />
                                    ) : (
                                        <CollectionGrid className="library-grid assets-library-grid" style={{ "--assets-grid-columns": gridDensity } as React.CSSProperties}>
                                            {canCreateAsset ? (
                                                <button type="button" className="library-create-card" onClick={openCreate}>
                                                    ...
                                                    <span className="library-create-title">新增资产</span>
            ...
            <Modal
                ...
                cancelText="取消"
            >
                确定将已选择的 {selectedAssets.length} 个资产移入回收站吗？移入后可随时在回收站批量还原。
            </Modal>
            <Modal
                className="library-modal library-confirm-modal"
                title="彻底删除资产"
            ...
            <Modal
                ...
                cancelText="取消"
            >
                确定彻底删除已选择的 {selectedAssets.length} 个资产吗？未被复用的服务器文件会同步删除，操作不可恢复。
            </Modal>
        </>
    );
}
        ...
        <AssetLibraryCard selected={selected}>
            ...
            <button type="button" className="block w-full px-2.5 py-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--workspace-accent)]" onClick={onOpen}>
                ...
                <div className="mt-1 flex min-w-0 items-center gap-1.5 text-[var(--fs-tiny)] text-foreground/38">
                    <span className="truncate">{assetSourceLabel(asset.source)}</span>
...
function AssetCover({ asset, selected, isTrash = false, onSelect, onOpen, menuItems }: { asset: LibraryAsset; selected: boolean; isTrash?: boolean; onSelect: (selected: boolean) => void; onOpen: () => void; menuItems: MenuProps["items"] }) {
    ...
    const KindIcon = kind ? assetKindIcons[kind] : UserRound;
    const clock = (asset.kind === "video" || asset.kind === "audio") && asset.data ? formatAssetClock(asset.data.durationMs) : null;
    const showPlay = asset.kind === "video";
    const isLight = asset.kind === "audio" || asset.kind === "text" || asset.kind === "model" || asset.kind === "entity" || asset.category === "character";
    return (
        <AssetLibraryCardMedia className={isLight ? "assets-cover is-light" : "assets-cover"}>
            <button type="button" className="assets-cover-link" onClick={onOpen} aria-label={`查看资产：${asset.title}`}>
                {asset.kind === "audio" ? (
                ...
                ) : asset.kind === "entity" || asset.category === "character" ? (
                    <CharacterCover asset={asset as Asset & { kind: "entity" }} />
                ) : (
                    <AssetMediaPreview
                        asset={asset}
                        alt={asset.title}
            ...
            <Dropdown trigger={["click"]} menu={{ items: menuItems }}>
                <button type="button" className="assets-cover-more" aria-label="更多资产操作" title="更多操作">
...
function ModelCover({ asset }: { asset: LibraryAsset & { kind: "model" } }) {
    return (
        <div className="assets-cover-model">
            <Box />
            <span>{asset.data?.fileName || "未命名模型"}</span>
        </div>
    );
}

function CharacterCover({ asset }: { asset: Asset & { kind: "entity" } }) {
    const definition = asset.data?.definition as Record<string, unknown> | undefined;
    const role = definition?.role as string | undefined;
    const appearance = definition?.appearance as string | undefined;
    const representations = (asset as Asset & { representations?: Array<{ url?: string }> }).representations;
    const coverUrl = asset.coverUrl && /^https?:\/\//i.test(asset.coverUrl) ? asset.coverUrl : "";
    const imageUrl = coverUrl || representations?.find((rep) => rep.url && /^https?:\/\//i.test(rep.url))?.url || "";
    if (imageUrl) return (
        <CachedResourceImage storageKey={imageUrl} src={imageUrl} alt={role || appearance || "角色卡"} loading="lazy" decoding="async" className="assets-cover-media" fallback={<div className="assets-cover-model"><UserRound /><span>{role || appearance || "角色卡"}</span></div>} />
    );
    return (
        <div className="assets-cover-model">
            <UserRound />
            <span>{role || appearance || "角色卡"}</span>
        </div>
    );
}
...
function AssetDrawer({ asset, onClose, onCopy, onDownload }: { asset: LibraryAsset | null; onClose: () => void; onCopy: (asset: LibraryAsset) => void; onDownload: (asset: LibraryAsset) => void }) {
    const [mediaInfo, setMediaInfo] = useState<{ width: number; height: number; bytes: number } | null>(null);
    const facts = asset ? assetArchiveFacts(asset, mediaInfo) : [];
    ...
    return (
        <Drawer className="library-drawer" title="资产档案" open={Boolean(asset)} size="large" onClose={onClose}>
            {asset ? (
                    ...
                    <div className="asset-archive-preview">
                        {asset.kind === "text" ? (
                            ...
                            <div className="asset-archive-audio">
                                <audio src={asset.data?.url || ((asset.data?.storageKey && /^https?:\/\//i.test(asset.data.storageKey)) ? asset.data.storageKey : "")} controls />
                            </div>
                        ) : asset.kind === "model" ? (
                            <div className="asset-archive-preview-model">
                                <Box />
                                <span>
                                    {asset.data?.fileName || "未知"} · {formatBytes(asset.data?.bytes || 0)}
                                </span>
                            </div>
                        ) : asset.kind === "video" ? (
                            <AssetArchiveVideo asset={asset} />
                        ) : (
                            <AssetImageZoom asset={asset} onInfo={setMediaInfo} />
                        )}
                    </div>
...
function AssetArchiveVideo({ asset }: { asset: LibraryAsset & { kind: "video" } }) {
    const rawStorageKey = asset.data?.storageKey || (asset as LibraryAsset & { storageKey?: string }).storageKey || "";
    const src = asset.data?.url || (rawStorageKey && /^https?:\/\//i.test(rawStorageKey) ? rawStorageKey : "") || "";
    if (!src) {
        return <div className="asset-archive-preview-note">视频地址不可用，请检查资产存储配置</div>;
    }
    return <CachedResourceVideo storageKey={rawStorageKey || src} src={src} controls preload="metadata" playsInline className="asset-archive-preview-media" />;
}

function AssetImageZoom({ asset, onInfo }: { asset: LibraryAsset & { kind: "image" }; onInfo?: (info: { width: number; height: number; bytes: number }) => void }) {
    const [scale, setScale] = useState(1);
    const [offset, setOffset] = useState({ x: 0, y: 0 });
    const dragRef = useRef<{ x: number; y: number; ox: number; oy: number } | null>(null);
    const reset = () => { setScale(1); setOffset({ x: 0, y: 0 }); };
    const rawStorageKey = asset.data?.storageKey || (asset as LibraryAsset & { storageKey?: string }).storageKey || "";
    const imageUrl = asset.coverUrl || asset.data?.dataUrl || (rawStorageKey && /^https?:\/\//i.test(rawStorageKey) ? rawStorageKey : "");
    const handleMediaLoad = (img: HTMLImageElement) => {
        const width = img.naturalWidth || asset.data?.width || 0;
        const height = img.naturalHeight || asset.data?.height || 0;
        const bytes = asset.data?.bytes || 0;
        onInfo?.({ width, height, bytes });
        const fetchUrl = rawStorageKey || imageUrl;
        if ((!bytes || bytes === 0) && fetchUrl && /^https?:\/\//i.test(fetchUrl)) {
            fetch(fetchUrl).then((resp) => resp.blob()).then((blob) => onInfo?.({ width, height, bytes: blob.size })).catch(() => {});
        }
    };
    return (
        <div className="asset-zoom-viewer" onWheel={(event) => { event.preventDefault(); setScale((value) => Math.min(4, Math.max(.25, value * (event.deltaY < 0 ? 1.12 : .89)))); }} onPointerDown={(event) => { if (scale <= 1) return; event.currentTarget.setPointerCapture(event.pointerId); dragRef.current = { x: event.clientX, y: event.clientY, ox: offset.x, oy: offset.y }; }} onPointerMove={(event) => { const drag = dragRef.current; if (!drag) return; setOffset({ x: drag.ox + event.clientX - drag.x, y: drag.oy + event.clientY - drag.y }); }} onPointerUp={() => { dragRef.current = null; }} onPointerCancel={() => { dragRef.current = null; }}>
            {imageUrl && <img src={imageUrl} alt={asset.title} loading="lazy" decoding="async" onLoad={(event) => { handleMediaLoad(event.currentTarget); }} className="asset-archive-preview-media asset-zoom-image" style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${scale})` }} />}
...
function assetArchiveFacts(asset: LibraryAsset, mediaInfo?: { width: number; height: number; bytes: number } | null) {
    const facts: Array<{ label: string; value: string }> = [
        { label: "类型", value: assetKindLabel(asset.kind) },
        { label: "分类", value: assetCategoryLabel(asset.category) },
    ];
    if (asset.kind === "image" || asset.kind === "video") {
        facts.push({ label: "尺寸", value: `${mediaInfo?.width || asset.data?.width || 0}x${mediaInfo?.height || asset.data?.height || 0}` });
    }
    if (asset.kind === "video" || asset.kind === "audio") {
        facts.push({ label: "时长", value: asset.data ? (formatAssetClock(asset.data.durationMs) || "未知") : "未知" });
    }
    if (asset.kind !== "text") {
        facts.push({ label: "大小", value: formatBytes(mediaInfo?.bytes || asset.data?.bytes || 0) });
        facts.push({ label: "格式", value: asset.data?.mimeType || "unknown" });
        const storageKey = asset.data?.storageKey || (asset as LibraryAsset & { storageKey?: string }).storageKey || (asset.coverUrl && /^https?:\/\//i.test(asset.coverUrl) ? asset.coverUrl : "") || "";
        facts.push({ label: "存储", value: storageKey ? resourceStorageLabel(storageKey) : "未知" });
    }
    facts.push({ label: "来源", value: assetSourceLabel(asset.source) });
...
function assetSummary(asset: LibraryAsset) {
    if (!asset.data) return "数据加载中...";
    if (asset.kind === "text") return asset.data.content || "无内容";
    if (asset.kind === "audio") return `${formatAssetDuration(asset.data.durationMs)} · ${formatBytes(asset.data.bytes || 0)} · ${asset.data.mimeType || "unknown"}`;
    if (asset.kind === "model") return `${asset.data.fileName || "未命名"} · ${formatBytes(asset.data.bytes || 0)} · ${asset.data.mimeType || "unknown"}`;
    if (asset.kind === "entity" || asset.category === "character") {
        const definition = asset.data.definition as Record<string, unknown> | undefined;
        return definition?.role ? `${definition.role}` : definition?.appearance ? `${definition.appearance}` : "角色卡";
    }
    return `${asset.data.width || 0}x${asset.data.height || 0} · ${formatBytes(asset.data.bytes || 0)} · ${asset.data.mimeType || "unknown"}`;
}
...
function assetSearchText(asset: LibraryAsset) {
    const definition = (asset.kind === "entity" || asset.category === "character") ? (asset.data?.definition as Record<string, unknown> | undefined) : undefined;
    return [
        asset.title,
        asset.source || "",
        asset.note || "",
        ...
        (asset.tags || []).join(" "),
        asset.kind === "text" ? (asset.data?.content || "") : 
        asset.kind === "entity" || asset.category === "character" ? 
            `${definition?.role || ""} ${definition?.appearance || ""} ${definition?.consistencyPrompt || ""}` : 
        (asset.data?.mimeType || "")
    ].join(" ").toLowerCase();
}
