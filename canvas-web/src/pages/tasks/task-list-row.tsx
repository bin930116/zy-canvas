...
export function TaskListRow({
    ...
    const isActive = task.status === "queued" || task.status === "running" || task.status === "processing";
...
function TaskPreviewThumbnail({ task, onOpen }: { task: GenerationTask; onOpen: () => void }) {
    ...
    return (
        <button
            ...
            aria-label={previewUnavailable ? "预览不可用，资产可能已删除" : isVideo ? "放大预览生成视频" : "放大预览生成图片"}
            title={previewUnavailable ? "预览不可用，资产可能已删除" : undefined}
        >
