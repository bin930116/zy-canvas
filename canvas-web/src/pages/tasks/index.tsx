...
export default function TasksPage() {
    ...
    const filteredTasks = useMemo(() => tasks.filter((task) => {
        ...
        if (statusFilter === "active") return task.status === "queued" || task.status === "running" || task.status === "processing";
    ...
    const taskStats = useMemo(() => {
        ...
        for (const task of tasks) {
            ...
            if (task.status === "queued" || task.status === "running" || task.status === "processing") active += 1;
    ...
    useEffect(() => {
        ...
        const poll = async (initial = false) => {
            ...
            const hasActiveTasks = items.some((task) => task.status === "queued" || task.status === "running" || task.status === "processing");
    ...
    const deleteLocalTask = (task: GenerationTask) => {
        ...
        Modal.confirm({
            title: "删除本机任务记录？",
            content: "这只会删除本机任务记录，不会删除已生成的资产。",
            okText: "删除本机记录",
...
function reconcileTaskSummaries(current: GenerationTask[], next: GenerationTask[]) {
    ...
    const reconciled = next.map((task) => {
        ...
        if (previous?.updatedAt === task.updatedAt && previous.status === task.status && previous.stage === task.stage && previous.progress === task.progress && previous.resultJson === task.resultJson && previous.previewUrl === task.previewUrl && previous.previewPosterUrl === task.previewPosterUrl && previous.billing?.status === task.billing?.status && previous.billing?.amountMicrocredits === task.billing?.amountMicrocredits) return previous;
        changed = true;
        return task;
    });
