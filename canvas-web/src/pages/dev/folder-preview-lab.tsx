...
export default function FolderPreviewLab() {
    ...
    const addSampleContent = () => {
        ...
        setChildrenByFolder((current) => {
            ...
            const next = makeMediaNode(`${openedId}-asset-${Date.now()}`, `新资产 ${index + 1}`, SAMPLE_MEDIA[index % SAMPLE_MEDIA.length], index);
        ...
        <main className="folder-preview-lab">
            <header className="folder-preview-lab-header">
                <div>
                    ...
                    <h1>通用资产文件夹</h1>
                    <p>文件夹外形与主题皮肤独立：四套主题都不是内部资产。打开后，真实内容以独立卡片展示；悬停、单击、双击与菜单均可交互。</p>
                </div>
