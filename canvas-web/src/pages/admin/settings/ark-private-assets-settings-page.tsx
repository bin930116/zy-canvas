...
export default function ArkPrivateAssetsSettingsPage() {
    ...
    const load = useCallback(
        async (initial = false, announce = false) => {
            ...
            try {
                ...
                if (!isAdminArkPrivateAssetSetting(result.setting)) throw new Error("服务端返回的方舟资产库配置格式无效");
                setSetting(result.setting);
                setDirty(false);
                setSaveError("");
                if (announce) message.success("已重新读取方舟资产库配置");
            } catch (error) {
                if (requestVersion !== requestVersionRef.current) return;
                const errorMessage = error instanceof Error ? error.message : "读取方舟资产库配置失败";
    ...
    const save = async (values: ArkPrivateAssetForm) => {
        ...
        try {
            ...
            if (!isAdminArkPrivateAssetSetting(result.setting) || !arkPrivateAssetResponseMatches(result.setting, expected, expectedHasSecret)) {
                throw new Error("服务端返回的方舟资产库配置与本次保存内容不一致，请重新读取后核对");
            }
            setSetting(result.setting);
            const nextValues = toFormValues(result.setting);
            form.setFieldsValue(nextValues);
            setDirty(false);
            message.success("方舟资产库配置已保存");
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : "保存方舟资产库配置失败";
    ...
    if (loading && !setting) {
        return (
            <AdminPageFrame title="方舟资产库" description="为 Seedance 参考图配置后端可信资产导入" scroll>
                <div className="admin-settings-stack admin-ark-settings" aria-label="正在读取方舟资产库配置" role="status">
                ...
                <div className={cn("admin-ark-command-bar", dirty && "is-dirty")}>
                    <div className="admin-ark-command-copy" aria-live="polite">
                        <div className="flex flex-wrap items-center gap-2">
                            <strong>{dirty ? "方舟资产库有调整待保存" : "方舟资产库配置已同步"}</strong>
                ...
                >
                    <div id="admin-ark-credentials" className="admin-settings-anchor">
                        <SettingsSectionCard
                            ...
                            description="凭据仅保存在服务端，用于创建资产组、上传图片和查询审核状态。"
                    ...
                    {prerequisitesReady || draftEnabled ? (
                        <div id="admin-ark-policy" className="admin-settings-anchor">
                            <SettingsSectionCard
                                ...
                                title="2. 是否启用可信资产同步"
                                description="启用后，符合条件的 Seedance 参考图会在生成前导入方舟资产库。"
                            ...
                            >
                                <div className="admin-ark-policy-control">
                                    <div className="admin-ark-policy-copy">
                                        <strong>自动导入可信参考资产</strong>
                                        <p>关闭只停止新资产同步，不删除已有资产和绑定。</p>
                                    </div>
                                    <div className="admin-ark-policy-switch">
                                        <span>{draftEnabled ? "启用" : "停用"}</span>
                                        <Form.Item name="enabled" valuePropName="checked" noStyle>
                                            <Switch aria-label="启用可信资产同步，保存修改后生效" />
                                        </Form.Item>
                                    </div>
                                </div>
                            </SettingsSectionCard>
...
function validateArkPrivateAssetDraft(values: ArkPrivateAssetForm, setting: AdminArkPrivateAssetSetting) {
    ...
    if (!draft.accessKeyId) return "请填写方舟资产库 IAM AccessKey";
    if (!hasUsableSecret(draft, setting)) return draft.accessKeyId !== setting.accessKeyId ? "更换 IAM AccessKey 时请同时填写匹配的 SecretKey" : "请填写方舟资产库 IAM SecretKey";
    return "";
}
