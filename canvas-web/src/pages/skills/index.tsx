...
import { BookOpenText, Boxes, Check, Clapperboard, Heart, Library, LoaderCircle, Megaphone, MoreHorizontal, Palette, Plus, Puzzle, Search, ShoppingBag, Sparkles, UserRound } from "lucide-react";
...
import { PromptTemplatePanel } from "@/pages/skills/prompt-template-panel";
...
const scopeOptions = [
    { label: "提示词模板", value: "templates", icon: BookOpenText },
    ...
    return (
        <>
            <WorkspacePage className="library-page skills-library-page" grid>
                ...
                <ListToolbar className="library-toolbar skills-toolbar mt-7" active={filtersActive} onReset={resetFilters}>
                    <div className="skills-tabs" ref={tabsRef} role="tablist" aria-label="技能库范围">
                        ...
                        {scopeOptions.map((option) => {
                            ...
                            const active = option.value === "templates" ? templatePanel : scope === option.value;
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => { if (option.value === "templates") { setTemplatePanel(true); return; } setTemplatePanel(false); setScope(option.value as SkillScope); setPage(1); }}
                                >
                ...
                {templatePanel ? <PromptTemplatePanel /> : loading && !skills.length ? <SkillSkeleton /> : loadError ? <WorkspaceErrorState compact description={loadError} onRetry={reload} /> : groupedSkills.length ? (
                ...
                {templatePanel ? null : <PaginationBar current={page} pageSize={pageSize} total={total} pageSizeOptions={[20, 40, 80]} onChange={(nextPage, nextPageSize) => { setPage(nextPageSize !== pageSize ? 1 : nextPage); setPageSize(nextPageSize); }} />}
            </WorkspacePage>
