...
export function RunningHubSettingsPane() {
    ...
    const fetchWorkflow = async () => {
        ...
        try {
            ...
            setTitle(item.title || "");
            message.success(
                workflowNeedsMediaUpload(item.fields || []) ? `${kind === "app" ? "RunningHub App" : "RunningHub 工作流"}参数已保存；生成时会调用 RunningHub 资产上传接口` : `${kind === "app" ? "RunningHub App" : "RunningHub 工作流"}参数已重新拉取并保存`,
            );
        } catch (error) {
        ...
        <Form layout="vertical" requiredMark={false}>
            <div className="settings-pane-header">
                <div className="min-w-0">
                    ...
                    <p>独立于系统渠道配置。工作流提交使用积分 API Key，参考资产上传使用企业级 API Key。</p>
                </div>
            ...
            <div className="settings-section grid gap-3 lg:grid-cols-12">
                ...
                <Form.Item label="资产上传 API Key（企业级）" className="mb-0 lg:col-span-6" extra="仅用于上传参考图片、视频、音频和蒙版；没有参考资产时可以留空。">
