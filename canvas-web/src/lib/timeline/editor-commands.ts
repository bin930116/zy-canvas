...
import type { SrtEntry, SubtitleHighlight, TimelineClip, TimelineProject, TimelineTrack, TimelineTrackKind } from "@/types/timeline";
...
export type RebuildSubtitleClipsPayload = {
    ...
    trackId?: string;
    /** 可选重点词高亮；按 entryIndex 写入对应字幕 clip，供预览叠字上色。 */
...
function handleRebuildSubtitleClips(state: TimelineProject, payload: unknown): TimelineProject {
    const { nodeId, entries, trackId, highlights } = payload as RebuildSubtitleClipsPayload;
    ...
    const rebuilt: TimelineClip[] = entries.map((entry) => {
        const highlight = highlights?.find((item) => item.entryIndex === entry.index);
        return {
            ...
            text: entry.text,
            ...(highlight ? { subtitleHighlight: highlight } : {}),
        };
    });
