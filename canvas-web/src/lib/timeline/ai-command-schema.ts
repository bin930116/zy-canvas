...
export const AI_EDITING_OP_CATALOG: ReadonlyArray<{
...
}> = [
    {
        ...
        payload: "{ clip: { id(新唯一), kind: video|audio|image|text, trackId(必须存在), nodeId(源资产), startMs≥0, durationMs>0, title?, volume?, fadeInMs?, fadeOutMs? } }",
    },
    {
        op: "moveClip",
        desc: "移动片段（时间点/跨轨）",
...
export function buildAiEditingSystemPrompt(timelineSummary: string): string {
    ...
    return [
        "你是诚迈科技视频剪辑工作台的 AI 剪辑助手。根据用户指令对时间线输出受约束的编辑命令 JSON。",
        "时间线当前状态：",
        timelineSummary,
