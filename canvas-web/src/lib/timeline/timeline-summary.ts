...
export function summarizeTimeline(project: TimelineProject): string {
    ...
    lines.push(`时间线：总时长 ${formatMs(project.durationMs)}，${project.tracks.length} 条轨道，${project.clips.length} 个片段（字幕 ${subtitleClips.length}，直连资产 ${directCount}，引用节点 ${nodeIds.size} 个）。`);
