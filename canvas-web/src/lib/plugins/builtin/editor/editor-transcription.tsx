...
// 上传资产的底层对象以 resource:<id> 形式存储在资源表，取其中的资源 ID 提交转写。
...
export function EditorTranscription() {
    ...
    const transcribe = async () => {
        ...
        if (!resourceId) {
            setFailed(true);
            setMessage("该资产没有可转写的媒体对象（缺少 storageKey）");
            return;
        }
        setRunning(true);
        setFailed(false);
            ...
            <div className="director-scroll min-h-0 flex-1 overflow-y-auto p-3">
                {transcribableAssets.length === 0 ? (
                    <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
                        ...
                        <p className="text-xs text-[var(--director-dock-fg)]/60">项目暂无音视频资产</p>
                        <p className="max-w-[200px] text-[11px] leading-relaxed text-[var(--director-dock-fg)]/45">资产库加入音频或视频后可转写自动字幕</p>
                    </div>
                ) : (
                    <>
                        <label className="mb-2 block">
                            <span className="mb-1 block text-[11px] text-[var(--director-dock-fg)]/70">音视频资产</span>
                            <select
                                value={selectedId ?? ""}
