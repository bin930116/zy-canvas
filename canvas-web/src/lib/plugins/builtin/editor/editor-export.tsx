...
function collectRenderSources(project: TimelineProject): TimelineRenderSource[] {
    ...
    for (const clip of project.clips) {
        ...
        const storageKey = direct.storageKey || "";
        // Snowy 产物 storageKey 可能是 http 直链：导出 fetchSourceBlob 需要 url
        const url = direct.url || (/^https?:\/\//i.test(storageKey) ? storageKey : undefined);
        sources.push({
            nodeId: clip.nodeId,
