...
import { buildFallbackHighlights } from "@/lib/timeline/subtitle-highlights";
...
export function EditorSubtitleTools() {
    ...
    // 无画布 AI 时用本地回退高亮（终止标点首段），保证预览能上色。
    const importSrt = () => {
        ...
        const highlights = buildFallbackHighlights(entries);
        dispatch({
            op: "rebuildSubtitleClips",
            payload: { nodeId: DEFAULT_SRT_NODE_ID, entries, trackId: subtitleTracks[0]?.id, highlights },
        });
        setMessage(`已从节点 ${DEFAULT_SRT_NODE_ID} 重建 ${entries.length} 条字幕快照${highlights.length ? `（${highlights.length} 条高亮）` : ""}`);
    };
