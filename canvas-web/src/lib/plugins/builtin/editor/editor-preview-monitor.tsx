...
import { CanvasSubtitleOverlay } from "@/components/canvas/canvas-subtitle-overlay";
...
import { createDefaultSubtitleStyle, type TimelineClip, type TimelineProject } from "@/types/timeline";
...
/** 预览叠字默认样式（与画布字幕弹窗默认高亮色一致） */
const defaultPreviewSubtitleStyle = createDefaultSubtitleStyle();
...
export function EditorPreviewMonitor() {
    ...
    // 当前时间点命中的字幕（可见字幕轨；优先带 highlight 的片段）
    const activeSubtitleClips = useMemo(() => {
        if (!project) return [];
        const hidden = new Set(project.tracks.filter((t) => t.visible === false).map((t) => t.id));
        return project.clips.filter(
            (clip) =>
                clip.kind === "subtitle" &&
                !hidden.has(clip.trackId) &&
                clip.startMs <= playbackMs &&
                playbackMs < clip.startMs + clip.durationMs &&
                (clip.text || "").trim(),
        );
    }, [playbackMs, project]);
    ...
    // 源资产比片段短：video 提前播完 → 把时钟对齐到片段结尾，让后续片段/结束自然衔接。
            ...
            <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-black p-6">
                {showEmpty ? (
                    ...
                    <div className="relative flex max-h-full max-w-full items-center justify-center">
                        ...
                        {activeSubtitleClips.length > 0 ? (
                            <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-md">
                                {activeSubtitleClips.map((clip) => (
                                    <CanvasSubtitleOverlay
                                        key={clip.id}
                                        text={(clip.text || "").trim()}
                                        highlight={clip.subtitleHighlight}
                                        style={defaultPreviewSubtitleStyle}
                                    />
                                ))}
                            </div>
                        ) : null}
