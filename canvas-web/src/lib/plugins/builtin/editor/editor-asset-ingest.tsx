...
// 展示项目资产库，按来源区分「本地上传」与「项目资产」；点击资产 → makeClipFromAsset
...
import { uploadResourceFile, resourceFileUrl, resourceIdFromStorageKey, type ResourceUploadMeta } from "@/services/api/resources";
import { resolveMediaUrl } from "@/services/file-storage";
import type { ProjectAsset } from "@/services/api/projects";
import type { TimelineProject } from "@/types/timeline";

/** 角色卡等无 storageKey 的资产：从 representation.url / resourceId 解析可播地址。 */
function projectAssetMediaUrl(asset: ProjectAsset): string {
    if (asset.storageKey) {
        if (/^https?:\/\//i.test(asset.storageKey)) return asset.storageKey;
        const resourceId = resourceIdFromStorageKey(asset.storageKey);
        if (resourceId) return resourceFileUrl(resourceId);
    }
    const reps = asset.character?.representations || [];
    const preferred =
        reps.find((item) => item.role === "primary") ||
        reps.find((item) => item.role === "turnaround_sheet") ||
        reps.find((item) => item.role === "front") ||
        reps.find((item) => item.mediaType?.startsWith("image")) ||
        reps[0];
    if (!preferred) return "";
    if (preferred.url && /^https?:\/\//i.test(preferred.url)) return preferred.url;
    if (preferred.resourceId) return resourceFileUrl(preferred.resourceId);
    return "";
}

async function resolveProjectAssetUrl(asset: ProjectAsset): Promise<string> {
    if (asset.storageKey) {
        const resolved = await resolveMediaUrl(asset.storageKey, "").catch(() => "");
        if (resolved) return resolved;
    }
    return projectAssetMediaUrl(asset);
}
...
function AssetThumb({ asset }: { asset: ProjectAsset }) {
    const isImage = asset.mediaType === "image" || Boolean(asset.character);
    const [url, setUrl] = useState<string | null>(null);
    useEffect(() => {
        let alive = true;
        resolveProjectAssetUrl(asset)
    ...
    if (asset.mediaType === "video" && url) {
        return (
            <div className="grid size-9 shrink-0 place-items-center overflow-hidden rounded-md bg-black">
...
function MediaPreview({ asset }: { asset: ProjectAsset }) {
    ...
    if (asset.mediaType === "image" || asset.character) {
    ...
    return (
        <div className="flex h-16 items-center justify-center gap-2 rounded-md bg-[var(--director-control-hover)] text-[var(--director-dock-fg)]/60">
            ...
            <span className="text-[10px]">音频资产 · 点击下方按钮加入时间线</span>
        </div>
    );
}
...
function SourceBadge({ source }: { source: string }) {
    ...
    return (
        ...
        >
            {uploaded ? "本地上传" : canvas ? "画布资产" : "项目资产"}
        </span>
    );
}
...
export function EditorAssetIngest() {
    ...
    const importFiles = async (fileList: FileList | File[]) => {
        ...
        try {
            // 去重：同名同类型视为同一资产。库内已有或本批内重复的跳过，避免
            // 再次上传同一大文件生成重复 asset（历史上重复导入会把资产堆积成多条）。
            ...
            if (okCount > 0) {
                // 刷新后校验本次挂载的资产是否都出现在列表里；若单次刷新因网络抖动
                ...
                setImportNote(`媒体库中已有同名资产，跳过 ${skipped} 个重复文件`);
            }
            if (firstErrorMsg) {
    ...
    // 双击“添加到时间线”会连续 dispatch 两次，产生两个同一资产的相邻片段；用时间戳守卫拦截 1.5s 内的重复添加。
    const addToTimeline = (asset: ProjectAsset) => {
        ...
        // 角色卡按图片入轨（用 representation 图）
        const kind = asset.mediaType === "video" ? "video" as const
            : asset.mediaType === "audio" ? "audio" as const
            : asset.mediaType === "image" || asset.character ? "image" as const
            : null;
        if (!kind) return;
    ...
    const groups: { id: "uploaded" | "project" | "canvas"; label: string; icon: typeof HardDrive; items: ProjectAsset[] }[] = [
        ...
        { id: "canvas", label: "画布资产", icon: Boxes, items: canvasAssets },
        { id: "project", label: "项目资产", icon: Boxes, items: projectAssets },
    ];
                ...
                {assets.length === 0 ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
                        <p className="text-xs text-[var(--director-dock-fg)]/60">
                            {filter === "uploaded" ? "暂无本地上传的媒体" : filter === "canvas" ? "暂无画布资产，请先在画布工作台生成分镜/预演" : filter === "project" ? "暂无项目资产" : "项目暂无资产"}
                        </p>
                        <p className="max-w-[180px] text-[11px] leading-relaxed text-[var(--director-dock-fg)]/45">点击上方导入媒体，或将文件拖入此区域</p>
                    </div>
                ) : (
                                                        ...
                                                        {expanded ? (
                                                                ...
                                                                <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[10px]">
                                                                    ...
                                                                    <dt className="text-[var(--director-dock-fg)]/60">来源</dt>
                                                                     <dd className="text-right text-[var(--director-dock-fg-strong)]">{source === SOURCE_UPLOADED ? "本地上传" : source === SOURCE_CANVAS ? "画布资产" : "项目资产"}</dd>
                                                                    {asset.durationMs ? (
                                                                        <>
