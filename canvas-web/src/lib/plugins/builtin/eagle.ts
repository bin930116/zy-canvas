...
const eaglePluginDocumentation = `# Eagle 资产库

Eagle 插件把桌面 Eagle 资料库接入站点资产面板。浏览器不会直接访问 Eagle：所有读取、搜索、下载和写入请求先到站点后端，再由后端访问管理员配置的 Eagle Local API 地址。
...
4. 保存配置后，在资产库选择 Eagle 来源，即可浏览和导入资产。
...
插件读取 Eagle 文件夹树并生成完整层级路径。资产列表支持 \`folderId\`、\`keyword\`、\`limit\`、\`offset\`。文件夹列表在当前插件实例内缓存；创建文件夹后会失效并重新读取。切换 Eagle 资料库时应重新打开插件设置确认目录。
...
| Eagle 内容 | 站点结果 | 当前限制 |
| --- | --- | --- |
| 图片 | 图片资产 | 使用原图，不把缩略图当成品 |
| 视频 | 视频资产 | 元数据缺宽高时使用 1280×720 展示兜底，不改写原文件 |
| 音频 | 音频资产 | 时长以浏览器媒体探测结果为准 |
| 其他文件 | 模型资产 | 仅作为文件资源保存 |
| 文本/实体 | 不支持 | 导入会明确报错 |

## 写回 Eagle

插件可把站点图片、视频或音频转换为 data URL，再调用 Eagle 添加资产。已有本地 Blob 优先直接读取；只有没有本地副本时才下载远程生成地址。默认单个手动上传文件不超过 96 MB，且 MIME 必须以 \`image/\`、\`video/\` 或 \`audio/\` 开头。
...
export const eagleAssetPlugin: RegisteredPlugin = {
    manifest: {
        ...
        description: "把 Eagle 作为站点的外部资产来源，直接浏览原始文件夹并读写 Eagle 文件。",
...
async function importEagleAsset(item: ExternalAssetItem, baseUrl: string, signal?: AbortSignal): Promise<Asset> {
    ...
    if (kind === "text" || kind === "entity") throw new Error("当前 Eagle 导入暂不支持文本或实体资产");
    ...
    if (kind === "image") {
        ...
        return {
            id: nanoid(), kind, title: item.title, coverUrl: uploaded.url, tags: item.tags || [], source: "Eagle 资产库", createdAt: now, updatedAt: now, metadata,
    ...
    if (kind === "video") {
        return {
            id: nanoid(), kind, title: item.title, coverUrl: "", tags: item.tags || [], source: "Eagle 资产库", createdAt: now, updatedAt: now, metadata,
    ...
    return {
        id: nanoid(), kind: "model", title: item.title, coverUrl: "", tags: item.tags || [], source: "Eagle 资产库", createdAt: now, updatedAt: now, metadata,
