...
function normalizeAssetData(asset: Asset): Asset["data"] {
    ...
    if (asset.kind === "video") {
        ...
        const httpStorageKey = storageKey && /^https?:\/\//i.test(storageKey) ? storageKey : undefined;
        const resolvedUrl = typeof raw.url === "string" && raw.url ? raw.url : httpStorageKey || asset.coverUrl || "";
        if (typeof raw.url === "string" && typeof raw.width === "number" && typeof raw.height === "number") return asset.data;
        return { url: resolvedUrl, storageKey, width: numberValue(raw.width), height: numberValue(raw.height), durationMs: numberValue(raw.durationMs), hasAudio: raw.hasAudio === true, bytes: numberValue(raw.bytes), mimeType: typeof raw.mimeType === "string" ? raw.mimeType : "video/*" };
    }
    if (asset.kind === "audio") {
        const storageKey = typeof raw.storageKey === "string" ? raw.storageKey : undefined;
        const httpStorageKey = storageKey && /^https?:\/\//i.test(storageKey) ? storageKey : undefined;
        if (typeof raw.url === "string") return asset.data;
        return { url: httpStorageKey || "", storageKey, durationMs: numberValue(raw.durationMs), bytes: numberValue(raw.bytes), mimeType: typeof raw.mimeType === "string" ? raw.mimeType : "audio/*" };
    }
    if (asset.kind === "model") {
...
export function parseAssetStorageDocument(value: string | null, fallback: Asset[] = []): AssetStorageDocument {
    ...
    if (!Array.isArray(parsed.state?.assets)) throw new Error("资产持久状态无效");
