...
import { type CapabilitySpec, type OptionConstraint, type PublicChannelCatalog, type PublicLogicalModel } from "@/services/api/logical-models";
...
export async function applyUserSession(payload: AuthSessionPayload) {
    ...
    try {
        ...
        if (!persistedConfig) {
            // TODO(模型模块): 模型目录接口 /model-catalog 尚未在 Snowy 侧实现，暂不发起查询，
            // 直接使用默认空配置；等模型模块开发完成后，再恢复 getModelCatalog() 生成初始渠道的逻辑。
            useConfigStore.getState().replaceConfig(normalizeConfigSnapshot({ config: defaultConfig }).config);
        }
        // TODO(模型模块): 已有配置时合并最新系统渠道的逻辑暂不执行，待模型模块接入后恢复。
...
export async function refreshSystemChannels() {
    // TODO(模型模块): 模型目录接口 /model-catalog 尚未实现，暂不拉取/合并系统渠道；接入后恢复。
    return;
}
