...
async function downloadResourceBlob(storageKey: string, target: ResourceCacheMeta) {
    // resource: 前缀走资源文件接口；HTTP 直链（dev 下载链接 / 对象存储直链）直接 fetch。
    const blob = target.resourceId ? await getResourceBlob(storageKey) : await fetchRemoteBlob(storageKey);
    if (!blob) return null;
    sessionBlobs.set(target.key, blob);
...
async function fetchRemoteBlob(url: string) {
    try {
...
async function cacheTarget(storageKey: string): Promise<ResourceCacheMeta | null> {
    ...
    // 带签名的 HTTP 直链（dev 下载链接 / 对象存储直链）：同一 URL 内容不可变，按 URL 缓存，
    // 避免角色卡/资产图每次刷新都重新下载。
    if (/^https?:\/\//i.test(storageKey)) {
        const version = "url";
        return {
            key: `${userScope}:http:${version}:${storageKey}`,
            userScope,
            resourceId: "",
            version,
            size: 0,
