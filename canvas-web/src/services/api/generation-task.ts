...
export async function runBackendToolGenerationTask(options: {
    ...
    const { apiBaseURL } = await import("@/services/api/request");
    const base = String(apiBaseURL).replace(/\/+$/, "");
    const snowyToken = localStorage.getItem("snowy_token") || "";

    // 同步 Agent 接口：POST /api/agent/chat → SSE
    const response = await fetch(`${base}/api/agent/chat`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Accept: "text/event-stream",
            ...(snowyToken ? { token: snowyToken } : {}),
        },
        credentials: "include",
        signal: options.signal,
        body: JSON.stringify({
            model,
            messages: buildBackendToolRequests(options.messages, options.tools, options.toolChoice, options.config).chatCompletion.messages,
            tools: options.tools,
    ...
    if (!response.ok) {
        ...
        throw new Error(`Agent 接口调用失败: ${response.status} ${errText.slice(0, 300)}`);
    }
    ...
    const handleEvent = (eventName: string, data: string) => {
        try {
            ...
            if (eventName === "delta" && typeof payload.content === "string") {
                ...
                throw new Error(payload.message || "Agent 调用失败");
            }
        } catch (err) {
...
async function localGenerationReferences(images: ReferenceImage[], videos: ReferenceVideo[], audios: ReferenceAudio[]): Promise<LocalDreaminaGenerationInput["references"]> {
    const imageReferences = await Promise.all(
        images.map(async (image) => {
            ...
            if (!source && !image.storageKey) throw new LocalDreaminaGenerationClientError("dreamina_reference_invalid", "即梦图片参考资产不可用", 400);
...
function invalidLocalReference() {
    return new LocalDreaminaGenerationClientError("dreamina_reference_invalid", "即梦参考资产无效", 400);
}
...
function assertBackendRuntimeConfigured(config: AiConfig, mode: BackendGenerationMode) {
    // 文本/图像/视频任务均由后端按 DB 模型配置直连（图像走百炼原生网关/OpenAI兼容，视频走 NewAPI 网关），
    // 不依赖前端渠道协议，故全部放行；前端仅保留本地模型与 workflow 相关校验
    if (mode === "text" || mode === "image" || mode === "video") return;
