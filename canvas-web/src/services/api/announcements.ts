...
/** 系统消息（站内信）：与公告分离 */
export function getSystemMessageFeed(limit = 20) {
    return request<SystemMessageFeed>(api.get("/announcements/messages", { params: { limit } }));
}

export function getAnnouncementDetail(id: string) {
    return request<SystemAnnouncement>(api.get("/announcements/detail", { params: { id } }));
}
