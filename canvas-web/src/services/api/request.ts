...
export const apiBaseURL = import.meta.env.VITE_CANVAS_BACKEND_URL || "";
export const apiClient = axios.create({ baseURL: apiBaseURL });

// Sa-Token 请求拦截器：在每次请求头中注入 token
...
// 401 响应拦截器：清除 token 并跳转登录
// 登录页自身调 getLoginUser 未登录时也会401，不能在这里清 token 否则死循环
apiClient.interceptors.response.use(
    (response) => response,
    (error) => {
        const isLoginPage = window.location.pathname.startsWith("/login");
        if (!isLoginPage && (error.response?.status === 401 || error.response?.data?.code === 401)) {
