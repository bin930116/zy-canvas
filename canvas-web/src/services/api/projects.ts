...
/**
 * 项目设置里的默认生图/视频模型优先于全局默认。
 * 模型可能来自后端 zy_model 目录，前端渠道列表未必包含，不能因渠道校验丢掉项目默认。
 */
export function resolveProjectGenerationModel(
    project: Pick<Project, "defaultImageModel" | "defaultVideoModel"> | undefined | null,
    globalModel: string | undefined,
    capability: "image" | "video",
): string {
    const projectModel = (capability === "video" ? project?.defaultVideoModel : project?.defaultImageModel)?.trim();
