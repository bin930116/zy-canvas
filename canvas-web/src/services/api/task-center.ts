...
export type TaskStatus = "queued" | "running" | "processing" | "succeeded" | "failed" | "cancelled";
...
const defaultGenerationTaskListDependencies: GenerationTaskListDependencies = {
    ...
    listLocalPage: async (page, signal) => {
        // 本地渠道关闭(默认)时不探测本机运行时，避免 127.0.0.1:17371 连接报错
        if (!useUserStore.getState().features.desktopLocalChannelsEnabled) {
            return { tasks: [] };
        }
        return listLocalDreaminaGenerationTaskPage(page, {}, signal);
    },
};
