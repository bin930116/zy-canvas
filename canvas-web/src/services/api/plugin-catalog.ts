...
type PluginProviderCatalogItem = {
    ...
    models?: Array<{ id: string; name: string; capability: string }>;
};
...
export async function fetchPluginProviderCatalogRaw(scope: string, capability?: ProtocolCapability) {
    const result = await request<{ providers: PluginProviderCatalogItem[] }>(apiClient.get("/plugins/catalog", { params: { scope, capability } }));
    return result.providers.filter((item) => item.enabled && !item.unavailableReason);
}
