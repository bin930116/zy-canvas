...
export async function uploadImage(input: string | Blob): Promise<UploadedImage> {
    // 优先直传 Snowy 后端（/files 上传图片接口），成功即返回可访问 URL；后端不可用时退回本地兜底。
    try {
        return await uploadImageToSnowyBackend(input);
    } catch {
        // 继续走原本地/资源系统路径
    }
...
// 直传 Snowy 后端上传图片接口：POST /files，返回可直接访问的 URL 与 storageKey。
async function uploadImageToSnowyBackend(input: string | Blob): Promise<UploadedImage> {
    const blob = typeof input === "string" ? await (await fetch(input)).blob() : input;
    const previewUrl = URL.createObjectURL(blob);
    const meta = await readImageMeta(previewUrl).catch(() => ({ width: 0, height: 0, mimeType: "" }));
    URL.revokeObjectURL(previewUrl);
    const formData = new FormData();
    formData.append("file", blob, input instanceof File ? input.name : "image.png");
    ...
    const data = await request<{ id: string; filename: string; storageKey: string; mimeType: string; bytes: number; url: string; width?: number; height?: number }>(
        apiClient.post("/files", formData),
    );
    if (!data.url) throw new Error("后端未返回图片访问地址");
    return {
        url: data.url,
        storageKey: data.storageKey || `image:${getActiveUserScope()}:${nanoid()}`,
        width: data.width || meta.width || 0,
        height: data.height || meta.height || 0,
        bytes: data.bytes || blob.size,
        mimeType: data.mimeType || blob.type || meta.mimeType,
    };
}
