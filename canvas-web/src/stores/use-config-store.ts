...
export type RunningHubConfig = {
    ...
    apiKey: string;
    walletApiKey: string;
    /** 仅用于 RunningHub 参考资产上传，通常填写企业级 API Key。 */
    uploadApiKey?: string;
    useWallet: boolean;
    capability: RunningHubCapability;
...
function isAiConfigReady(config: AiConfig, model: string) {
    ...
    // 仅选择了模型标识即视为就绪：Snowy 集成下 baseUrl/apiKey 在 zy_model 后端配置，不要求前端渠道
    if (model?.trim()) {
        const channel = resolveModelChannel(config, model);
        // 前端渠道有完整配置时按渠道判断；否则允许后端模型（后端按 modelKey 查库）
        if (channel.baseUrl.trim() && channel.apiKey.trim()) return true;
        return true;
    }
    return false;
}
