...
/** 仅时间线作用域的直连媒体（不落画布）：资产库/项目资产/本地上传在时间线内直接入轨时使用。 */
export type TimelineDirectMedia = {
    /** 源资产 id（本地资产 id / 上传文件 id），用于生成时间线内唯一 nodeId 前缀 */
    id: string;
    /** 远端资产库记录；本地上传成功后必须存在。 */
    assetId?: string;
...
export type TimelineClip = {
    ...
    durationMs: number;
    title?: string;
    /** 源资产内裁剪起点（毫秒），用于导出阶段定位源文件片段 */
    sourceStartMs?: number;
    /** 源资产总时长（毫秒），用于 UI 限制裁剪上限 */
    ...
    text?: string;
    /** 单条字幕的重点词高亮（预览叠字用；由 SRT 导入回退或画布 AI 生成后写入快照） */
