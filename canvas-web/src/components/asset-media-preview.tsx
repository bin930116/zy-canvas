...
export function AssetMediaPreview({ asset, alt, className = "", fallback = null }: AssetMediaPreviewProps) {
    ...
    const storageKey = asset.kind === "image" ? (asset.data.storageKey || (asset as Asset & { storageKey?: string }).storageKey) : undefined;
    const imageUrl = asset.coverUrl || (asset.kind === "image" ? asset.data.dataUrl : "") || (storageKey && /^https?:\/\//i.test(storageKey) ? storageKey : "");
