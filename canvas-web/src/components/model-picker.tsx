...
import { fetchPluginProviderCatalogRaw } from "@/services/api/plugin-catalog";

type CatalogProviderEntry = { id: string; name: string; vendor: string; models: Array<{ id: string; name: string; capability: string }> };

const catalogFetchCache = new Map<string, Promise<CatalogProviderEntry[]>>();

function fetchCatalogOnce(capability?: ModelCapability): Promise<CatalogProviderEntry[]> {
    const key = capability || "all";
    let promise = catalogFetchCache.get(key);
    if (!promise) {
        promise = fetchPluginProviderCatalogRaw("user.custom-channel", capability)
            .then((providers) => providers.map((provider) => ({ id: provider.id, name: provider.name, vendor: provider.vendor || "", models: (provider.models || []).map((model) => ({ id: model.id, name: model.name, capability: model.capability })) })))
            .catch(() => [] as CatalogProviderEntry[]);
        catalogFetchCache.set(key, promise);
    }
    return promise;
}
...
export function ModelPicker({
    ...
    const [catalogProviders, setCatalogProviders] = useState<Array<{ id: string; name: string; vendor: string; models: Array<{ id: string; name: string; capability: string }> }>>([]);
    // 后端模型目录（zy_model）：本地模式/无渠道都拉；Snowy 集成下渠道可能为空
    useEffect(() => {
        let cancelled = false;
        fetchCatalogOnce(capability).then((providers) => {
            if (!cancelled) setCatalogProviders(providers);
        });
    ...
    // 但项目默认模型 / 后端 zy_model 目录模型可能不在前端渠道列表：有显式 value 时保留展示，避免被清成「未选择」。
    const catalogModelIds = useMemo(() => new Set(catalogProviders.flatMap((provider) => provider.models.map((model) => model.id))), [catalogProviders]);
    const current = resolvedCurrent && (options.includes(resolvedCurrent) || catalogModelIds.has(resolvedCurrent) || Boolean(storedCurrent)) ? resolvedCurrent : "";
    // 查找 catalog 中的模型名称（用于显示）
    const catalogModelName = useMemo(() => {
        for (const provider of catalogProviders) {
            for (const model of provider.models) {
                if (model.id === storedCurrent) return model.name;
            }
        }
        return null;
    }, [catalogProviders, storedCurrent]);
    ...
    const setPickerOpen = (nextOpen: boolean) => {
        if (nextOpen) {
            // 打开时也拉一次目录，避免挂载前失败缓存空列表
            fetchCatalogOnce(capability).then((providers) => setCatalogProviders(providers));
        }
    ...
    const content = (
        <div
            ref={menuRef}
            data-canvas-no-zoom
            className={cn("canvas-model-picker-menu max-w-[calc(100vw-24px)]", creationVariant ? "creation-model-picker-menu" : "w-[var(--panel-width-compact)]")}
            style={
                {
                    background: theme.node.panel,
                    color: theme.node.text,
                    ...(creationVariant ? { width: "420px", minWidth: "420px" } : {}),
        ...
        >
            ...
            {optionGroups.length || catalogProviders.length ? (
                <>
                    ...
                    {catalogProviders.map((provider) => (
                        <section key={provider.id} className="canvas-model-picker-group min-w-0 overflow-hidden">
                            <div className="canvas-model-picker-group-label" style={{ color: theme.node.muted }}>
                                <span className="truncate">{provider.name}</span>
                                <span className="shrink-0 text-xs">{provider.vendor}</span>
                            </div>
                            <div className="grid min-w-0 gap-1">
            ...
            >
                ...
                >
                    <span className="canvas-model-picker-label flex min-w-0 items-center gap-1.5">
                        ...
                        <span className="min-w-0 flex-1 truncate">{current ? (catalogModelIds.has(current) && catalogModelName ? catalogModelName : creationVariant ? pickerModelDisplayName(config, current, showConfiguredModelName) : pickerModelOptionLabel(config, current, showConfiguredModelName)) : catalogModelName || placeholder}</span>
