...
export function AssetLibraryPickerModal({
    ...
    title = "资产库",
    eyebrow = "参考内容",
    confirmLabel = (count) => `使用已选资产${count ? `（${count}）` : ""}`,
    emptyTitle = "这个分类还没有资产",
    ...
    const remoteItems = useMemo<AssetLibraryPickerItem[]>(() => (remoteQuery.data?.assets || []).filter((asset) => asset.kind !== "entity" && asset.kind !== "model").map((asset) => ({
        ...
        ...(items.find((item) => item.id === asset.id) || { disabledReason: "此资产不适用于当前操作" }),
    ...
    const confirm = async () => {
        ...
        try {
            ...
            setError(reason instanceof Error ? reason.message : "资产操作失败，请重试");
        } finally {
            setWorking(false);
        }
    };
    ...
    const handleRestoreSelected = async () => {
        ...
        try {
            ...
            setSelected(new Set());
            message.success(`已还原 ${archivedSelectedIds.length} 个资产至资产库`);
            setCategory("all");
        } catch {
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            setSelected(new Set());
            message.success(`已彻底删除 ${archivedSelectedIds.length} 个资产`);
        } catch (err) {
    ...
    const handleEmptyRecycleBin = async () => {
        ...
        try {
            ...
            setSelected(new Set());
            message.success(`已删除${remoteEnabled ? "当前页" : "回收站"} ${toDelete.length} 个资产`);
            setCategory("all");
        } catch (err) {
    ...
    const handleUpload = async (files: FileList | null) => {
        ...
        try {
            ...
            setError(reason instanceof Error ? reason.message : "资产上传失败，请重试");
        } finally {
    ...
    const sourceLabel = source === "plugin" ? "插件来源" : "本地资产";
            ...
            <div className="asset-picker-shell">
                <header className="asset-picker-toolbar">
                    <div className="asset-picker-heading">
                        <div className="asset-picker-heading-copy">
                            ...
                            >
                                <button type="button" className="asset-picker-title-trigger" aria-haspopup="menu" aria-expanded={sourceMenuOpen} aria-label={"资产库来源：" + sourceLabel}>
                    ...
                    <label className="asset-picker-search">
                        <Search aria-hidden />
                        <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索资产名称或标签" aria-label="搜索资产" />
                    </label>
                    <span className="asset-picker-count">
                        已选 {selectedIds.length} · {effectivePagination ? effectivePagination.total : visibleItems.length} 个资产
                    </span>
                </header>
                <div className="asset-picker-body">
                    <nav className="asset-picker-categories" aria-label="资产分类">
                            ...
                            <>
                                ...
                                {normalCategories.map((value) => (
                                    <button key={value} type="button" className={cn("assets-filter-item", category === value && "is-active")} aria-pressed={category === value} onClick={() => setCategory(value)}>
                                        <span className="assets-filter-item-label">{categoryLabels[value] || (value === "all" ? "全部资产" : "其他")}</span>
                    ...
                    <div className="asset-picker-grid-wrap">
                        <div className="asset-picker-grid">
                            {remoteEnabled && remoteQuery.isError ? <div role="alert">资产读取失败<Button onClick={() => void remoteQuery.refetch()}>重试</Button></div> : loading || (remoteEnabled && remoteQuery.isFetching) ? (
                                <div className="asset-picker-empty">
                                    <LoaderCircle className="animate-spin" />
                                    <strong>正在读取资产</strong>
                                    <span>资产会按页加载，不会一次下载整个项目库。</span>
                                </div>
                            ) : visibleItems.length ? (
                                ...
                                <div className="asset-picker-empty">
                                    ...
                                    <span>{isRecycleBin ? "删除画布或手动归档的资产会暂存到这里，可在需要时还原。" : activeUpload ? "换个分类，或从底部上传一份新资产。" : emptyDescription}</span>
                                </div>
                            )}
                        </div>
                ...
                <footer className={cn("asset-picker-footer", !activeUpload && "is-compact")}>
                    {activeUpload ? (
                        <>
                            ...
                            <button type="button" className="asset-picker-upload" onClick={() => uploadInputRef.current?.click()} disabled={working} aria-busy={uploading}>
                                ...
                                <span>
                                    <strong>{uploading ? `正在上传 ${uploadingCount} 个资产` : "上传新资产"}</strong>
                    ...
                    <div className="asset-picker-actions">
                        {isRecycleBin ? (
                            <>
                                <Popconfirm title={remoteEnabled ? "确认删除当前页回收站资产？" : "确认清空回收站？"} description="仅删除当前列表中的资产；仍被引用的资产由服务端拒绝删除。删除不可恢复。" onConfirm={handleEmptyRecycleBin} okText="删除" okButtonProps={{ danger: true }} cancelText="取消">
                                    <Button type="text" danger disabled={working || !archivedCount}>
                                        {remoteEnabled ? "删除当前页" : "清空回收站"}
                                    </Button>
                                </Popconfirm>
                                <Popconfirm title="确认彻底删除已选资产？" onConfirm={handleDeleteSelected} okText="删除" okButtonProps={{ danger: true }} cancelText="取消">
                                ...
                                <Button type="primary" icon={<RotateCcw className="size-3.5" />} disabled={working || !archivedSelectedIds.length} loading={working} onClick={handleRestoreSelected}>
                                    还原已选资产{archivedSelectedIds.length ? `（${archivedSelectedIds.length}）` : ""}
                                </Button>
                            </>
                        ) : (
                            <>
        ...
        <AssetLibraryCard selected={selected} className={cn("asset-picker-card", disabled && "is-disabled")}>
            <button type="button" className="asset-picker-card-action" onClick={onToggle} disabled={disabled} aria-pressed={selected} title={item.disabledReason || item.title}>
                ...
                <div className="asset-picker-card-copy">
                    <strong>{item.title || "未命名资产"}</strong>
