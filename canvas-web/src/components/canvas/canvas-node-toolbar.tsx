...
export function CanvasNodeToolbar({
    ...
    const processMenuLabel = compact ? "工具" : isVideo ? "提取资产" : isImage ? "图片工具" : isAudio ? "音频处理" : "文本调整";
...
function nodeGenerationRows(node: CanvasNodeData) {
    ...
    if (metadata.references?.length) {
        ...
        add("引用资产", `${metadata.references.length} 个${referenceNames ? `（${referenceNames}${metadata.references.length > 3 ? "…" : ""}）` : ""}`);
    }
