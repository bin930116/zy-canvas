...
import { modelDisplayName, modelIcon, normalizeModelOptionValue, resolveModelRequestConfig, selectableModelsByCapability, useConfigStore, useEffectiveConfig, type AiConfig } from "@/stores/use-config-store";
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition(
        ...
        "创建语义化工作流/流水线：节点使用真实的文本、脚本、图片、视频或音频类型；character_cards=角色拆分图片卡片，character_three_view=角色三视图，storyboard_video=分镜剧情视频。工具会自动生成唯一 id、按节点实际尺寸布局、创建 edges/referenceRefs/referenceNodeIds 连线、选择新节点并复核重叠。媒体节点必须提供有意义的 prompt 或 content；已有资产先 canvas_find_nodes/canvas_get_resources，再把真实 node id 放入 referenceNodeIds。不要用 canvas_create_text_nodes 代替工作流。",
        {
            title: { type: "string" },
...
export function CanvasAssistantPanel({
    ...
    const sendMessage = async (text: string, history: CanvasAssistantMessage[], savedReferences?: CanvasAssistantReference[]) => {
        const fallbackModel = "qwen3.8-flash";
        const model = (effectiveConfig.textModel || effectiveConfig.model || "").trim() || fallbackModel;
    ...
    const runOnlineAgentStep = async (sessionId: string, assistantId: string, history: CanvasAssistantMessage[], userMessage: CanvasAssistantMessage, loop: OnlineLoopContext) => {
        const requestConfig = { ...effectiveConfig, model: (effectiveConfig.textModel || effectiveConfig.model || "").trim() || "qwen3.8-flash" };
        try {
            setIsRunning(true);
    ...
    const continueOnlineToolLoopAfterResults = async (sessionId: string, assistantId: string, messages: ResponseInputMessage[], toolCalls: ResponseToolCall[], toolResults: OnlineExecutedToolCall[], step: number) => {
        ...
        const requestConfig = { ...effectiveConfig, model: (effectiveConfig.textModel || effectiveConfig.model || "").trim() || "qwen3.8-flash" };
        let streamed = "";
        const next = await requestOnlineAgentModel({ ...requestConfig, systemPrompt: "" }, nextMessages, "required", "继续处理画布工具结果", (text) => {
            streamed = text;
            if (text.trim()) upsertMessage(sessionId, { id: assistantId, role: "assistant", text });
        });
    ...
    const submitCinematicProject = async (text: string) => {
        ...
        if (!value) return;
        if (isRunning) {
            console.warn("[Agent] 上一轮仍在运行，强制开启新一轮");
            setIsRunning(false);
        }
            ...
            {view === "chat" ? (
                <>
                    ...
                    <AgentChatComposer
                        ...
                        left={
                            <>
                                <VoiceRecordingButton onTranscribed={(text) => setPrompt((prev) => (prev.trim() ? `${prev} ${text}` : text))} />
...
function AgentTextModelPicker({ config, value, onChange }: { config: AiConfig; value: string; onChange: (model: string) => void }) {
    ...
    useEffect(() => {
        let cancelled = false;
        void (async () => {
            const paths = ["/api/model-catalog", "/model-catalog"];
            let models: Array<{ id: string; name: string }> = [];
            for (const path of paths) {
                try {
                    const { request, apiClient } = await import("@/services/api/request");
                    const data = await request<{ providers?: Array<{ name?: string; models?: Array<{ modelKey?: string; modelName?: string; capability?: string }> }> }>(apiClient.get(path));
                    for (const provider of data?.providers || []) {
                        for (const model of provider.models || []) {
                            if (!model.modelKey) continue;
                            // text 优先；capability 缺省也收（后端可能只写 modelKey）
                            const cap = (model.capability || "").toLowerCase();
                            if (cap && cap !== "text") continue;
                            models.push({ id: String(model.modelKey), name: model.modelName || String(model.modelKey) });
                        }
                    }
            ...
            if (models.length) {
                ...
                // 兜底：后端目录不可用时，直接用已知的平台文本模型
                setCatalogModels([{ id: "qwen3.8-flash", name: "qwen3.8-flash" }]);
                if (!value) onChange("qwen3.8-flash");
            }
        })();
    ...
    const catalogOptions = useMemo(() => catalogModels.filter((m) => !options.includes(m.id)), [catalogModels, options]);
    const current = value || "";
    const allOptions = [
        ...options.map((model) => ({ value: model, label: modelDisplayName(config, model) })),
        ...catalogOptions.map((m) => ({ value: m.id, label: m.name })),
    ];
    return (
        <div className="min-w-0 max-w-[240px]" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <Select<string>
                ...
                optionRender={(option) => {
                    ...
                    const isCatalog = catalogOptions.some((m) => m.id === model);
                    return (
                        <span className="flex min-w-0 items-center gap-2">
                            {!isCatalog ? <AgentModelIcon config={config} model={model} /> : null}
                            <span className="min-w-0 flex-1 truncate">{isCatalog ? catalogModels.find((m) => m.id === model)?.name || model : modelDisplayName(config, model)}</span>
                        </span>
                    );
                }}
                labelRender={() => {
                    const isCatalog = catalogOptions.some((m) => m.id === current);
                    return (
                        <span className="flex min-w-0 items-center gap-1.5">
                            {!isCatalog && current ? <AgentModelIcon config={config} model={current} /> : null}
                            <span className="min-w-0 truncate">{current ? (isCatalog ? catalogModels.find((m) => m.id === current)?.name || current : modelDisplayName(config, current)) : "选择文本模型"}</span>
                        </span>
                    );
                }}
                onChange={onChange}
                aria-label="选择 Agent 文本模型"
                title={current || "选择文本模型"}
            />
        </div>
    );
}
