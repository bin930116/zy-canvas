    ...
    const dockItems: FloatingDockEntry[] = [
        {
            ...
            label: open ? "收起资产空间" : `打开资产空间，共 ${(showLibrary ? assetImages.length : 0) + canvasImages.length} 项`,
        ...
        <div ref={rootRef} data-canvas-no-zoom className="relative" style={{ zIndex }} onPointerDownCapture={bringToFront} onFocusCapture={bringToFront} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
            <AnimatePresence>
                {open ? (
                    ...
                    >
                        <button type="button" className="canvas-asset-tray-resize-handle absolute left-1/2 top-1 z-10 flex h-5 w-28 -translate-x-1/2 items-center justify-center rounded-full opacity-35 transition-opacity hover:opacity-75" onPointerDown={startResize} aria-label="从顶部调整资产托盘高度" title="拖动调整高度">
                        ...
                        <div className={cn("relative grid gap-1 rounded-[var(--r-lg)] p-0.5", showLibrary ? "grid-cols-2" : "grid-cols-1")} style={{ background: theme.spatial.surface }}>
                            {showLibrary ? <TrayTabButton active={tab === "library"} label={`资产库 ${assetImages.length}`} theme={theme} onClick={() => setTab("library")} /> : null}
                        ...
                        <label className="mt-2 flex h-8 items-center gap-1.5 rounded-[11px] px-2.5 focus-within:ring-2" style={{ background: theme.spatial.surface }}>
                            ...
                            <input type="search" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索图片资产..." className="min-w-0 flex-1 bg-transparent text-[var(--fs-tiny)] outline-none placeholder:opacity-55" aria-label="搜索图片资产" />
                        ...
                        <div className="thin-scrollbar mt-2.5 min-h-0 flex-1 overflow-y-auto pr-1">
                            {showLibrary && tab === "library" ? (
                                filteredAssets.length ? (
                                    ...
                                    <TrayEmpty text="没有匹配的图片资产" theme={theme} />
                                )
            ...
            <FloatingDock items={dockItems} className="canvas-floating-dock" style={canvasDockStyle(theme)} ariaLabel="资产空间" />
        </div>
    );
}
