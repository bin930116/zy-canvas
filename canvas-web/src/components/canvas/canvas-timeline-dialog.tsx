...
export function CanvasTimelineDialog({
    ...
    // 仅首次打开时构建草稿：资产上传/资产库插入会触发 nodes 变化，不能因此重建草稿丢弃编辑。
    useEffect(() => {
        if (!open) {
    ...
    // 工具栏自适应：按钮总数过多或容器变窄时，把「添加资产/上传本地/资产库/项目资产」收进「更多」菜单，
    // 工具栏保持单行不换行；容器变宽后自动恢复内联展示。
    // 资产组用隐藏探针测量自然宽度，展开/收起两种形态下算出的核心区宽度一致，判定不随当前状态震荡。
    ...
    // 把“添加节点”能力暴露给页面层（资产库/本地上传创建节点后回填到草稿）。
    useEffect(() => {
    ...
    const runExport = async (): Promise<Blob> => {
        ...
        if (!sources.length) throw new Error("找不到可导出的视频资产，请确认视频节点包含媒体");
        setExporting(true);
        setExportPercent(0);
    ...
    const title = (
        <div className="flex min-w-0 items-center gap-2.5 pr-10">
            ...
            <div className="min-w-0">
                ...
                <div className="truncate text-xs opacity-45">第二期 · 轨道编辑与资产编排</div>
            </div>
        </div>
    );
                        ...
                        <Dropdown
                            ...
                            menu={{
                                items: [
                                    ...
                                    { key: "library", label: "资产库", icon: <Library className="size-3.5" />, disabled: !onOpenAssetLibrary, onClick: () => onOpenAssetLibrary?.() },
