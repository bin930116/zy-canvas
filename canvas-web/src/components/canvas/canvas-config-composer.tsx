        ...
        >
            <div className="mb-3 flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-baseline gap-2">
                    ...
                    <div className="truncate text-[var(--fs-label)] opacity-55">{simpleMode ? "已连接资产会自动带入" : workflowVideoReferenceMode ? "已连接媒体会按工作流字段顺序自动带入" : "@ 引用已连接资产或已激活技能，发送前自动组装"}</div>
                </div>
            ...
            <div className="canvas-config-composer-editor relative rounded-lg" style={{ background: theme.node.fill }}>
                {!value.trim() ? <div className="pointer-events-none absolute left-4 top-3 text-sm leading-7" style={{ color: theme.node.placeholder }}>输入提示词，按 @ 引用连接资产或技能</div> : null}
                <div
                    ref={editorRef}
                    contentEditable
