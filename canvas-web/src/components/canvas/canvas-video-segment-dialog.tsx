            ...
            <div className="space-y-4">
                <div className="flex min-h-0 items-center justify-center overflow-hidden rounded-xl border bg-black" style={{ borderColor: theme.toolbar.border }}>
                    {videoUrl ? (
                        ...
                        <div className="grid h-40 w-full place-items-center text-xs opacity-60">{videoError ? "视频预览加载失败，请检查资产是否仍然可用" : "正在加载视频…"}</div>
                    )}
                </div>
                ...
                {isVideoMode ? (
                        ...
                        {createsGenerationNodes ? (
                            <>
                                ...
                                <label className="block">
                                    ...
                                    <div className="mt-1 text-xs opacity-45">每个片段会连接一个待生成视频节点，但不会自动提交任务。你可以继续调整参考资产、模型和提示词。</div>
                                </label>
                            </>
                        ) : (
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-lg border px-3 py-2 text-sm" style={{ borderColor: theme.toolbar.border }}>
                        ...
                        <div className="w-full text-xs opacity-45">提取的 MP3 会保存到资产库，并生成一个音频节点放在当前视频节点下游。</div>
                    </div>
                )}
