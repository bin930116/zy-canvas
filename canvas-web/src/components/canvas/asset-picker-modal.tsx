...
const categoryLabels: Record<string, string> = { all: "全部资产", ...ASSET_CATEGORY_LABELS, archived: "回收站" };
...
export function AssetPickerModal({ open, multiple = true, onInsert, onClose }: Props) {
    ...
    return (
        <AssetLibraryPickerModal
            ...
            multiple={multiple}
            confirmLabel={(count) => `插入已选资产${count ? `（${count}）` : ""}`}
            emptyDescription="先在资产库中添加图片、视频、音频或文本。"
            onClose={onClose}
            onConfirm={async (ids) => {
...
export function assetPickerItemsToInsertPayloads(ids: string[], items: AssetLibraryPickerItem[]): InsertAssetPayload[] {
    ...
    return ids.map((id) => {
        ...
        if (!pickerItem) throw new Error("所选资产已不存在，请重新选择");
        ...
        if (!asset || (asset.kind !== "text" && asset.kind !== "image" && asset.kind !== "video" && asset.kind !== "audio")) {
            throw new Error(`“${pickerItem.title}”不是可插入画布的资产`);
        }
...
export function externalAssetToInsertPayload(reference: ExternalAssetPickerReference): InsertAssetPayload {
    ...
    if (item.kind === "image") return { kind: "image", dataUrl: url, url, title: item.title || "资产图片", width: item.width, height: item.height, bytes: item.bytes, mimeType: item.mimeType, assetId };
